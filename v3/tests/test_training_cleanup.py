...
from train import training_batch_limit
...
def test_pretraining_windows_stay_inside_source_samples(tmp_path):
    ...
    dataset = KoJamoDataset(data_dir=str(data_dir), seq_length=4, stride=2)
    ...
    for index in range(len(dataset)):
        ...
        assert "\n" not in dataset.tokenizer.decode(y)
...
def test_reasoning_router_solves_structured_jamo_tasks_without_hijacking_chat():
    ...
    assert try_reasoning_answer("오늘 기분이 어때?") is None
