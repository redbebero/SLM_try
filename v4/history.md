...
## 140. 최소 GRU 학습 붕괴 원인 분리 및 학습 루프 보강 (진행 중)

### 문제 재현

6트랙 `Independent GRU(hidden=128, 2층, emb=16)`를 정제 corpus에서 3epoch,
epoch당 50 batch만 학습했다. Validation loss는 `16.01 → 11.66`으로 내려갔지만,
자유생성은 모든 입력에서 `아아아아...`로 붕괴했다. `full_hangul=0.004`,
반복률 `0.973`, 20개 고정 질문 keyword 적중률 `0%`였다.

### 원인 분리

* 전체 학습은 150 step뿐인데 기존 warmup은 300 step으로 고정되어, 짧은 실험의
  절반 이상이 유효 학습률에 도달하기 전에 끝났다.
* pretrain 문장 끝의 파일 개행은 `strip()`으로 제거되어 모델이 문장 종료(EOS)를
  학습하지 못했다. 생성기는 종료 신호 없이 확률이 높은 음절을 계속 출력했다.
* 기존 `free_running` validation은 실제 예측을 다음 입력으로 되먹이지 않고,
  정답 입력 window에 대한 teacher-forced 평가에 가까웠다.
* 데이터 타입 분포는 한글 `69.0%`, 기호 `26.2%`, 숫자 `4.1%`, 영어 `0.7%`로,
  짧은 학습에서 타입 헤드가 한글 다수 클래스에 고정될 수 있었다.

### 적용한 변경

* `dataset.py`: pretrain 각 샘플 끝에 명시적 newline EOS를 추가하고,
  기존 EOS 없는 memmap cache를 자동 재생성하도록 `pretrain_eos` metadata를 추가했다.
* `train.py`: 유한 실험의 warmup을 전체 step의 5%로 제한하는
  `effective_warmup_steps()`를 추가했다. `--data-dir`, `--val-data-dir`로
  train/validation 폴더를 분리할 수 있게 했다.
  첫 구현은 전체 corpus의 batch 수로 warmup을 계산해 제한 실험에서도 300 step을
  사용했으나, `--limit-batches`를 반영하도록 즉시 수정했다. 현재 3epoch·50batch
  실험의 warmup은 7 step이다.
* `train.py`: 실제 예측을 다음 입력에 연결하는 `evaluate_autoregressive()`를
  추가해 token accuracy, type accuracy, 반복률을 별도 측정한다.
* `train.py`: best checkpoint와 ReduceLROnPlateau 기준을 기존 teacher-forced
  loss에서 `1 - autoregressive_token_acc + repeat_rate` generation score로 변경했다.
  Loss만 낮고 자유생성이 붕괴한 checkpoint를 best로 채택하던 문제를 차단한다.
* `chat.py`: 2층 GRU checkpoint의 layer 수를 잘못 추론하던 버그를 수정했다.
  기존 코드는 `weight_ih_l0` shape로 layer 수를 계산해 항상 1층으로 읽었다.

### 최소 toy 검증

모델: `Independent GRU`, `emb=16`, `hidden=64`, `layers=1`, `seq=8`,
30epoch, epoch당 20 batch. 학습·검증 문장을 분리하고 EOS를 포함했다.

* Epoch 27 autoregressive 결과: `token_acc=0.719`, `type_acc=1.000`,
  `repeat_rate=0.000`
* 생성 결과: `나는/너는/어머니가/강아지가 → 밥을 멁는다.`
* 결론: 자소·조사·어미·EOS는 학습 가능함을 확인했다. 그러나 주어에 따른
  의미 선택은 `밥을 멁는다` 한 문장으로 붕괴해, 의미 일반화는 아직 실패했다.

정제 corpus에서 EOS와 warmup을 적용한 동일 최소 모델(`hidden=128`, 2층,
3epoch·50batch)을 재실행했다. autoregressive `token_acc`는 기존 `0.002`에서
`0.039`로, 반복률은 `1.000`에서 `0.690`으로 개선됐지만 여전히 자유생성 통과 기준에
미달했다. 따라서 대규모 corpus에서는 구조보다 update 수 부족이 아직 주된 병목으로
남아 있다.

toy 조합 일반화 실험을 100epoch까지 연장했다(`hidden=64`, 1층, emb=16,
train 20문장 / validation 8문장, EOS 포함). Epoch 90~100에서 validation
autoregressive `token_acc=0.438`, `type_acc=0.875`, `repeat_rate=0.000`까지
안정화됐다. 그러나 `나는/너는/어머니가/아버지가/강아지가/고양이가/학생이/우리는`
모든 프롬프트에 `밥을 멁는다.`를 생성했다.

결론: 충분한 update를 주면 한국어 형식과 반복 제어는 회복되지만, 현재 데이터·loss·
짧은 context에서 주어 조건에 따른 의미 선택은 다수 패턴으로 붕괴한다. 이 실험은
모델 크기 부족의 증거가 아니라, 조건부 조합 학습을 별도로 설계해야 한다는 증거다.
다음에는 주어·목적어·동사 조합을 균등화하고, validation을 조합 단위로 나눠 조건부
정확도를 측정한다. Transformer 재실험과 hidden 확대는 그 후로 미룬다.

### 다음 검증

1. toy 조합 일반화에서 주어·목적어·동사별 조건부 정확도 측정
2. 타입 balanced sampling 및 scheduled sampling A/B
3. 정제 corpus에서 충분한 update 수로 재학습
4. 같은 예산으로 Independent GRU와 소형 Transformer 비교
5. 자유생성·EOS·반복률을 통과한 경우에만 모델 크기 확대

## 141. 균형 조합 toy 검증 실행 결과

앞선 toy는 주어 조건이 다수 패턴으로 붕괴했으므로, 주어 8개 × 목적어 4개의
문장 조합을 만들고 각 주어마다 1개 조합을 validation으로 홀드아웃했다.
학습 24문장, validation 8문장이다. 문장 형식은 다음과 같다.
...
* `Independent GRU`, `emb=16`, `hidden=64`, `layers=1`
* seq=8, batch=4, 120 epoch, EOS 포함
* train/validation 디렉터리 분리, seed=42
* checkpoint: `checkpoints/verify_compositionalbest.pth`

학습 중 aggregate autoregressive 지표는 마지막 구간에서
`token_acc=0.656`, `type_acc=1.000`, `repeat_rate=0.000`이었다. 그러나 이는
공백·기호·반복되지 않는 일반 토큰의 적중을 포함하므로 의미 조건 학습의 증거로
볼 수 없다.

실제 프롬프트 완성 검증에서는 학습 문장과 홀드아웃 문장 모두 실패했다.
`나는 책을 `, `너는 밥을 `, `어머니가 밥을 `, `아버지가 책을 ` 및 validation
프롬프트가 전부 `샇다 몹을 샇다`를 출력했다. validation 8개 exact answer hit는
`0/8`이다.

결론:

* 현재 최소 GRU는 한국어 형태 생성 및 반복 억제 일부는 학습하지만,
  주어·목적어에 따른 동사 조건을 학습하지 못했다.
* aggregate token accuracy는 이 목적에 부적합하다. 이후 조합 실험의 통과 기준은
  prompt별 exact completion, 동사/목적어 조건부 정확도, EOS 정상 종료로 한다.
* 데이터 조합을 균등화한 것만으로는 부족하다. 현재는 짧은 context와 독립
  자모 head의 조건부 결합 실패를 분리해야 한다. 다음 A/B는 (1) 문장 전체를
  충분히 포함하는 context/sequence 설정, (2) 자모 결합 일관성 검증 및
  syllable-level 출력 경로, (3) teacher forcing과 자유생성 간 차이 측정 순서로
  진행한다. 모델 크기 확대는 이 조건부 검증 통과 후로 보류한다.

## 142. sequence/context A/B 및 cache window 버그 수정

Section 141의 다음 실험으로 동일 toy에 `seq=16`, `seq=24`를 시도했다. 처음에는
기존 cache metadata에 `seq_length`와 `stride`가 저장되지 않아 `seq=8`용 window가
재사용되었고, batch 내부 tensor 길이가 달라져 다음 오류가 발생했다.
...
* `dataset.py` cache metadata에 `seq_length`, `stride` 저장
* 현재 설정과 cache 설정이 다르면 window cache 자동 재생성
* 동일 문제를 막는 회귀 검사 추가

회귀 검사는 별도 pytest 패키지가 없어 `python` 직접 assertion으로 실행했고
`cache_window_regression=PASS`를 확인했다.

그 후 확인한 toy 문장은 tokenizer 기준 EOS 포함 11 token뿐이었다. 따라서
`seq=16/24`는 유효 window가 0개이며 학습 불가하다. 이 데이터에서 가능한 최대
입력 길이인 `seq=10`으로 재실행했다.

`seq=10` 결과:

* `Independent GRU`, `emb=16`, `hidden=64`, `layers=1`, 120 epoch
* validation aggregate 마지막 구간: `free_type=0.800`,
  `free_cho=0.345`, `free_jung=0.552`, `free_jong=0.552`, `full_hangul=0.069`
* 실제 train/validation prompt exact completion: `0`
* 생성 예: ` ?*?"?"?` 또는 ` ?**?"?"`

결론: context 길이를 문장 전체에 맞춰도 현재 모델은 한국어 문장 생성을 회복하지
못했다. 문제의 주원인은 단순한 seq 부족이 아니다. aggregate 자모/토큰 지표와
실제 생성 품질의 괴리가 다시 확인됐다. 다음은 sequence 변경을 중단하고
syllable-level 출력 경로 또는 입력/출력 자모 결합 일관성부터 별도 toy로 검증한다.

## 143. 문자 단위 GRU baseline으로 자모 head 원인 분리

동일한 compositional toy 데이터(학습 24문장, validation 8문장)에 대해 6트랙
자모 출력을 사용하지 않는 유니코드 문자 단위 GRU baseline을 추가했다.

실험 설정:

* 문자 vocabulary: train/validation 전체 문자 집합
* `Embedding(16) → GRU(hidden=64, 1층) → Linear`
* AdamW, lr=`0.003`, 120 epoch, seed=42
* 문장 전체를 한 sequence로 학습하고 newline을 EOS로 사용

결과:

* validation loss: epoch 20 `0.2849`, epoch 120 `0.4272`
* validation prompt별 exact completion: **8/8**
* 주어·목적어가 학습에 없던 조합도 모두 올바른 동사 생성

예: `나는 밥을` → `나는 밥을 먹는다.`, `고양이가 책을` →
`고양이가 책을 던진다.`, `우리는 공을` → `우리는 공을 싫어한다.`

결론: 동일한 작은 GRU와 짧은 데이터에서 문자 단위는 조합 일반화에 성공했으므로,
현재 실패를 GRU 용량 부족이나 context 부족으로 설명할 수 없다. 주된 병목은
6트랙 자모 입력/출력 표현 또는 독립 자모 head의 학습·생성 결합 방식이다.
다음에는 문자 baseline을 기준선으로 고정하고, 자모 모델의 teacher-forced
다음문자 예측과 실제 `chat.generate` 경로를 같은 prompt에서 단계별 비교한다.

## 144. 기존 cascade decoder 재검증

조건부 자모 예측이 이미 `KoJamoNet(cascade=True)`로 구현되어 있어, 같은 toy와
최소 설정으로 별도 재실행했다.

* `Independent`와 동일: `emb=16`, `hidden=64`, `layers=1`, `seq=8`, 120 epoch
* validation 마지막 구간 aggregate autoregressive token 정확도는 약 `0.625`
* 그러나 실제 `chat.generate`에서 train/validation prompt 모두
  `맗을 맗을 맗을`로 생성
* 직접 exact completion: `0`

결론: 조건부 head를 단순히 켜는 것만으로는 해결되지 않는다. 현재 학습 중
autoregressive 평가와 실제 `chat.generate`의 결과가 계속 불일치한다. 다음 우선순위는
새 decoder 추가가 아니라, 같은 prompt의 (1) teacher-forced next-token logits,
(2) train.py free-running logits, (3) chat.generate logits를 단계별로 비교해
생성 경로의 입력/출력 정렬 또는 자모 선택 버그를 찾는 것이다.

## 145. autoregressive 지표 착시 및 실제 원인 확인

동일 cascade checkpoint에서 `chat.generate`와 raw model 출력을 같은 prompt로
비교했다. `나는 책을 `에 대해 모델의 첫 예측 type은 한국어였지만 자모 조합은
`삵`이었다. 즉 chat 경로가 prompt를 무시한 것이 아니라, 조건부 다음 음절을
잘못 예측하고 있었다.

더 중요한 문제는 `evaluate_autoregressive()`의 toy 평가 위치였다. 기본
`prompt_length=8`인데 toy 문장 `나는 책을 먹는다.`는 tokenizer 기준 11 token이고,
실제 의미 prompt `나는 책을 `은 6 token이다. prompt 길이 8은 이미 정답 동사의
일부를 입력으로 넣은 뒤 문장 후반부를 평가하므로 주어·목적어 조건 추론을
측정하지 않는다.

검증 결과:

* cascade aggregate metric: `prompt_length=8`에서 `token_acc=0.375`
* 의미 prompt에 가까운 `prompt_length=6`에서도 `token_acc=0.500`
* 실제 prompt별 exact completion: `0`
* raw 첫 예측: `나는 책을 ` → `삵`

결론: 지금까지의 aggregate autoregressive 점수는 의미 생성의 근거가 아니었다.
현재 모델은 prompt를 완전히 무시한다기보다, 자모 예측 분포가 문맥에 맞는
음절로 집중되지 않는다. 향후 통과 기준은 toy 문장에 대해 prompt 경계를 명시한
exact completion과 첫 정답 음절 적중률로 고정한다. 다음 수정은 평가부터
prompt-aware로 분리하고, 그 후 현재 jong 입력과 조건부 자모 decoder를 수정한다.

## 146. 현재 종성 입력 수정 A/B

입력 표현의 불일치를 수정했다. 기존 `KoJamoNet`과 `KoJamoTransformer`는 현재
음절의 초성·중성은 입력하면서 종성만 이전 timestep의 값을 사용하고 있었다.
`x_t → x_{t+1}` 예측에서는 현재 종성도 문맥이므로 두 모델 모두
`emb_jong(x[:, :, 2])`를 사용하도록 변경했다. 차원 수는 유지했다.

모델 smoke test는 GRU/Transformer 모두 `current_jong_input_smoke=PASS`였다.
이후 동일 toy에서 `Independent GRU`, `emb=16`, `hidden=64`, `layers=1`,
`seq=8`, 120 epoch를 재학습했다.

실제 prompt 결과:

* train/validation prompt 모두 동일하게 `싷아하다...` 형태로 붕괴
* exact completion: `0`

결론: 현재 종성 입력 누락은 수정해야 할 구조 결함이지만, 문맥 연관성 붕괴의
단독 원인은 아니다. 다음은 독립 자모 head의 factorized loss/decoding 자체를
분리 검증해야 한다. 현재 수정은 유지하되 checkpoint는 통과 모델로 채택하지
않는다.

## 147. 활성 생성 경로 축소 1차

원래 목표와 무관한 HRM/router/memory/intent 의존성이 `chat.py` 한 파일에 함께
들어 있어, 순수 자모 모델 실험 경로를 분리했다.

추가한 활성 경로:

```text
scripts/generate.py
  → scripts/core_generate.py
  → tokenizer.py + model.py
```

`core_generate.py`는 GRU/Transformer KoJamo checkpoint만 로드하고, HRM·Qwen·
memory·reasoning router를 import하지 않는다. `py_compile`과 실제 checkpoint
생성 smoke를 통과했다. 현재 checkpoint의 출력은 기존과 동일한
`싷아하다...` 형태이며, 이번 작업은 품질 개선이 아니라 실험 경로 격리다.

문자 baseline checkpoint를 자모 loader에 넣으면 명확한 구조 오류를 내도록
검사도 추가했다. 다음은 `train.py`를 active conditional-jamo 학습 경로로
축소하고, 기존 hybrid 파일은 legacy 보관 경로로 이동하는 단계다.

## 148. 활성 학습 경로 축소 2차

기존 `train.py`의 복잡한 variant/resume/HRM/Transformer 실험 분기를 대체할
최소 경로로 `scripts/train_core.py`를 추가했다.
...
train_core.py
  → dataset.py
  → KoJamoNet(cascade=True)
  → validation loss checkpoint
```

제거한 active 경로 기능:

* HRM·SFT·reasoning 분기
* Transformer 선택 분기
* 과거 checkpoint 자동 탐색
* 복잡한 resume/newstage 상태 복구
* hybrid router 연동

검증:

* 3 epoch smoke: 학습·validation·checkpoint 저장 통과
* 30 epoch toy: `train_loss=1.0473`, `val_loss=1.2778`
* `scripts/generate.py`: `나는 밥을` → `산다.`

결론: active 코드 경로는 짧아졌지만 자모 조건부 생성 실패는 그대로 재현됐다.
축소 과정에서 문제를 감추지 않았다는 뜻이다. `train_core.py`는 현재 cascade를
사용하므로, 다음에는 decoder/loss를 conditional three-hot joint 구조로 교체한다.
기존 `train.py`와 hybrid 스크립트는 아직 삭제하지 않고 legacy 이동 전 상태로
보존한다.

## 149. 전체 active 경로 실행 및 conditional decoder A/B

`model.py`에 기존 checkpoint와 호환되는 선택형 conditional decoder를 추가했다.
새 경로는 다음과 같다.

```text
context h
 → 초성 state/예측
 → 초성 embedding을 조건으로 중성 state/예측
 → 중성 embedding을 조건으로 종성 state/예측
```

`train_core.py`가 이 decoder를 사용하도록 연결했고, `core_generate.py`가 새
checkpoint 형식을 자동 판별하도록 수정했다. 개행 EOS에서 생성 중단도 추가했다.

실행 결과:

* conditional decoder smoke: `PASS`
* 30 epoch toy: `train_loss=0.8508`, `val_loss=1.0425`
* prompt-aware exact validation: `1/8`
* 대부분의 출력이 `산다.`로 수렴

따라서 구조 연결과 평가 경로는 정상이나, conditional decoder만 추가해서는
주어·목적어 조건을 충분히 학습하지 못했다. 현재 toy의 통과 모델로 채택하지
않는다.

현재 active 파일은 `train_core.py`, `core_generate.py`, `evaluate_core.py`,
`tokenizer.py`, `dataset.py`, `model.py`다. 기존 `train.py`, `chat.py`, HRM/SFT/
hybrid 스크립트는 다른 역사 실험과 import 관계가 있으므로 물리 삭제·이동하지
않고 legacy로 분류해 active 경로에서 제외했다. 강제 삭제하면 재현 경로가 깨질
수 있어 보존한다.

## 150. prompt-completion 학습으로 자모 모델 회복

Section 149의 conditional decoder만으로는 60 epoch exact `0/8`이었다. 원인을
decoder 하나로 단정하지 않고, 실제 사용 형태인 `prompt → answer`를 직접 학습하는
모드를 `train_core.py`에 추가했다.

prompt-completion 모드:

```text
입력: 나는 밥을
정답: 먹는다.
loss: 정답 시작 위치 이후에만 적용
```

문장 길이가 달라 validation batch stack이 실패하는 문제는 prompt 모드의 batch를
1로 고정해 해결했다. conditional decoder는 초성 후보별로 중성·종성을 다시
계산하는 joint candidate decoding을 추가했고, 각 조합을 다음 점수로 비교한다.

```text
log P(초성) + log P(중성|초성) + log P(종성|초성,중성)
```
...
* 60 epoch: exact `6/8`
* 120 epoch: exact **`8/8`**
* 모든 validation 조합의 동사를 정확히 생성

결론: 기존 실패의 큰 원인은 데이터에 연관성이 없어서가 아니라, random window
학습이 실제 semantic prefix completion을 충분히 직접 최적화하지 않았기 때문이다.
현재 conditional decoder + prompt-completion loss + joint candidate decoding 조합을
새 active toy 기준으로 채택한다. 다만 8개 toy validation 결과이므로 실제 corpus
일반화 증거는 아니며, 다음은 더 큰 조합 holdout과 clean corpus에서 같은 평가를
실행해야 한다.

## 152. 다운로드 corpus update·문장 길이 비교

동일 다운로드 subset(`train=2,000`, `valid=200`)에 대해 prompt-completion 학습을
3 epoch와 10 epoch로 비교했다.

* 3 epoch exact: `14/200`
* 10 epoch exact: `14/200`

update를 늘려도 개선되지 않았다. validation 출력은 `있습니다`, `됩니다` 같은
빈번한 종결 표현으로 수렴했다. 따라서 이 구간에서는 단순한 update 부족만으로
설명할 수 없다.

문장 길이 영향을 보기 위해 80자 이하 subset도 생성했으나 validation 문장이
0개였고, 120자 이하로 완화해도 validation이 2개뿐이었다. 이 subset은 비교
평가에 사용하지 않았다.

결론:

* 다운로드 corpus는 toy보다 훨씬 길고 표현·주제가 다양하다.
* 현재 64 hidden 소형 GRU에 전체 긴 문장을 바로 prompt-completion으로 넣으면
  흔한 어미 prior로 수렴한다.
* 다음에는 다운로드 corpus에서 충분한 validation을 유지한 길이 curriculum과
  prefix/answer 데이터 구성을 별도로 만든다.
* toy는 구조 통과 기준, 다운로드 corpus는 실제 일반화 기준으로 계속 분리한다.

## 153. 다운로드 corpus 길이 curriculum 실행

정제 corpus의 문장 길이 분포를 train/validation별로 조사했다. 짧은 문장은 거의
없었고, 유효한 첫 curriculum 구간으로 `121~180자`를 선택했다.
...
동일한 conditional GRU와 prompt-completion loss로 5 epoch 학습했다.

결과:

* exact: `18/111` (약 16.2%)
* 기존 2,000/200 subset 3·10 epoch: `14/200` (7.0%)

문장 길이를 제한한 curriculum에서 비율은 개선됐지만, 빈번한 종결어미로
수렴하는 현상은 남아 있다. 따라서 이 구간을 최종 데이터로 확정하지 않고,
다음에는 prefix 길이와 answer 길이를 제한한 prompt 데이터셋을 별도로 구성해
문장 전체를 외우는 것이 아니라 앞 문맥으로 뒷부분을 맞추는지 검증한다.

## 154. 다운로드 corpus prefix→다음 단어 검증

정제 corpus에서 기존 문장 전체를 answer로 사용하는 방식을 바꾸고, 단어 경계에서
다음 단어 하나만 정답으로 만드는 데이터셋을 생성했다.
...
동일한 conditional GRU와 prompt-completion loss로 학습했다.

* 5 epoch exact: `1/200`
* 20 epoch exact: `1/200`

update를 5배 늘려도 개선되지 않았다. toy의 `8/8` 성공은 단순한 한글 출력
능력과 작은 조합 규칙에 대한 결과이고, 실제 corpus의 긴 prefix에서 다음 단어를
정확히 선택하는 능력으로 바로 이어지지 않는다.

결론: 현재 hidden=64 소형 GRU와 긴 다운로드 prefix의 조합은 충분하지 않다.
다음에는 prefix 길이를 16~40자로 제한하고, answer를 한 음절/짧은 어미부터
시작하는 curriculum을 만든다. 동시에 validation answer의 train vocabulary
중복률을 검사해 OOV 문제가 아닌지 분리한다.

## 151. 다운로드 정제 corpus와 toy 비교

다운로드·정제되어 있던 `train_data_clean/pretrain_train.txt`와
`val_data_clean/pretrain_val.txt`를 실제 데이터 후보로 사용했다. 전체를 바로
학습하지 않고 deterministic subset을 만들었다.

```text
experiments/dataset_compare/train: 2,000문장
experiments/dataset_compare/valid: 200문장
```

통계:

* toy train/valid: 평균 11.2자, 한글 비율 약 73.3%
* 다운로드 subset train: 평균 273.8자, 한글 비율 57.4%
* 다운로드 subset valid: 평균 264.9자, 한글 비율 66.1%

동일한 `Conditional KoJamo GRU(emb=16, hidden=64, 1층)`와 prompt-completion
loss로 다운로드 subset을 3 epoch 학습했다.

결과:

* toy: 120 epoch exact `8/8`
* 다운로드 subset: 3 epoch exact `14/200`
* 다운로드 subset에서는 `있습니다`, `됩니다` 같은 빈번한 종결 표현으로 출력이
  수렴하는 현상이 확인됨

해석:

* 다운로드 데이터가 잘못된 것은 아니다. 문장이 길고 주제가 넓어 toy보다 훨씬
  어려우며, 3 epoch update로는 부족하다.
* toy는 구조·decoder·평가 검증용으로 유지한다.
* 실제 학습은 정제 다운로드 corpus를 사용하되, 짧은 문장/짧은 subset에서
시작해 update 수와 문맥 길이를 단계적으로 늘린다.
* raw 대화·roleplay·수학·위키 데이터를 한 번에 합치지 않는다. 각 데이터군의
  목적과 평가를 분리한다.

다음 active 학습 단계는 다운로드 subset에서 충분한 update를 주고,
긴 문장 random-window와 prompt-completion 혼합 비율을 비교하는 것이다.

## 155. 짧은 prefix curriculum 및 OOV 통제 검증

다운로드 정제 corpus에서 다음 데이터셋을 구성했다.
...
처음에는 validation 정답 중 train에 없는 항목이 있어 OOV 가능성을 확인했다.
builder를 수정해 validation answer가 train answer 집합에 포함되는 샘플만 선택했고,
최종 포함률은 `200/200`이었다.

동일 conditional GRU로 5 epoch 학습한 결과:

* prompt exact: `0/200`

결론: 짧은 prefix, 짧은 answer, answer vocabulary 중복을 보장해도 현재 hidden=64
자모 모델은 실제 corpus의 다음 단어를 학습하지 못했다. toy 성공과 실제 corpus
실패 사이에는 단순 OOV보다 문장 다양성·정답 빈도·모델 용량·학습 objective의
차이가 있다. 다음은 exact만 보지 말고 answer token accuracy와 빈도 baseline을
추가해, 모델이 정답 근처까지 가는지부터 측정한다.

## 157. answer 빈도 균형 sampling 및 EOS/길이 평가

기존 prefix curriculum checkpoint의 세부 지표를 측정했다.

```text
최빈 answer baseline: 10/200
model exact: 23/200
첫 글자 정확도: 33/200
문자 정확도: 46/407
EOS 정상 종료: 32/200
길이별 exact: 1자 13, 2자 9, 3자 1, 4자 0
```

answer별 빈도를 균등하게 만드는 `WeightedRandomSampler`를 추가해 A/B했다.
처음 구현에서 answer 첫 글자를 제외하고 빈도를 세는 오류가 발견되어 원문
answer를 보존하도록 수정한 뒤 재실행했다.
...
문자 정확도: 7/407
EOS: 11/200
상위 출력: '일이' 86, '일' 59, '일이을' 55
```

결론: 현재 hidden=64 자모 모델에서는 완전 균형 sampling이 희귀 answer를
안정적으로 학습시키지 못하고 출력 붕괴를 일으켰다. 균형 sampling은 채택하지
않는다. 기본 sampling을 유지하고, 다음은 answer 길이 curriculum과 더 큰
hidden/문맥 A/B로 진행한다.

## 156. prefix 모델 부분 정확도 및 빈도 baseline 측정

Section 155의 `prefix 16~40자`, answer 최대 4자 validation 200개를 대상으로
exact 외 지표를 추가했다.

```text
가장 흔한 train answer baseline: '그는' → 10/200 exact
모델 exact: 23/200
첫 글자 정확도: 33/200
문자 위치 정확도: 46/407
```

모델 출력 상위는 `가지` 50개, `가지고` 35개, `가` 27개, `그는` 23개였다.
따라서 모델은 단순 최빈 answer baseline보다는 낫지만, 특정 어근과 짧은 출력으로
강하게 수렴한다.

결론: 다음 개선은 모델 크기보다 answer 빈도 균형과 output length/EOS 학습을 먼저
검증한다. answer별 sampling weight를 적용한 A/B와 정답 길이별 정확도를 측정한다.

## 158. hidden 크기 및 answer 길이 curriculum A/B

동일한 OOV 통제 prefix 데이터에서 다음 세 조건을 비교했다.

```text
A: hidden=64, answer 최대 4자 → exact 23/200
B: hidden=128, answer 최대 4자 → exact 12/200
C: hidden=64, answer 최대 2자 → exact 23/200
```

C의 세부 결과는 첫 글자 `24/200`, 문자 정확도 `34/326`, EOS `107/200`이었다.
하지만 출력이 `두` 하나에 170개 몰렸다. B도 `가이`, `가` 중심으로 수렴했다.

결론: 현재 update 예산에서 hidden을 128로 키우는 것은 개선이 아니며, 정답을
짧게 제한하면 EOS는 좋아지지만 문맥 선택은 해결되지 않는다. 모델 확대보다
answer 분포·prefix 조건·출력 쏠림 원인을 먼저 정리한다.

## 159. 완성 음절 ID 직접 출력 구조 A/B

현재 자모 모델의 출력이 초성·중성·종성으로 따로 분해되어 잘못된 음절 조합이
생기는 문제를 줄이기 위해, 입력은 기존처럼 자모 정보를 포함하고 출력은 완성된
한글 음절/문자 ID 하나를 직접 고르는 작은 GRU를 추가했다.

구조:

```text
문자 ID 임베딩 + 초성/중성/종성 임베딩
→ tanh 선형층
→ 1층 GRU(hidden=64)
→ 완성 문자 ID softmax
```

동일한 설정(embedding=16, hidden=64, 1층, seed=42)으로 검증했다.

```text
toy validation: 8/8 exact
prefix 16~40 validation: 24/200 exact (12%)
```

기존 prefix 자모 모델의 23/200보다 1건 높아졌지만, 차이는 작다. 따라서
출력 음절 조합 문제는 일부 완화됐으나, 실제 데이터에서 성능을 제한하는 주된
원인은 여전히 prefix와 answer의 관계 부족, answer 빈도 쏠림, 긴 문맥을 작은
GRU가 유지하기 어려운 문제로 판단한다. 이 구조를 최종 확정하지 않고 다음
단계에서 데이터 조건과 학습 목표를 분리해 검증한다.

## 160. 완성 음절 모델 학습량 추가 검증

동일한 최소 구조에서 epoch만 5에서 10으로 늘렸다.

```text
5 epoch: 24/200 exact
10 epoch: 29/200 exact
10 epoch 최빈 답 baseline: 11/200
10 epoch 첫 글자: 43/200
10 epoch 문자 정확도: 64/407
10 epoch EOS: 154/200
```

학습량 증가는 toy가 아닌 실제 prefix 데이터에서도 소폭 개선을 만들었다. 그러나
정답 길이별 exact는 길이 1자 12건, 2자 15건, 3자 1건, 4자 0건으로 긴 답을
거의 학습하지 못한다. 따라서 다음 우선순위는 모델 확대가 아니라 answer 길이와
빈도를 통제한 데이터셋 재구성, 그리고 길이별 검증을 고정하는 것이다.

## 161. pytest 설치 및 전체 회귀 테스트

가상환경에 `pytest 9.1.1`을 설치하고 전체 테스트를 실행했다.
...
```text
scripts/test_inference.py: 과거 checkpoints/model_sft_v124.pth 없음
scripts/test_pretrain.py: 과거 checkpoints/model_v13.pth 없음
scripts/test_pointer.py: 현재 KoJamoNet에 과거 head_copy가 없음
tests/test_training_cleanup.py::test_sft_builder_loads_transformer_checkpoint:
  현재 tokenizer 종성 vocab=28, 저장된 과거 checkpoint 종성 vocab=29 불일치
tests/test_training_cleanup.py::test_reasoning_router...:
  현재 reasoning router가 일반 대화에도 답하도록 바뀌었으나 테스트는 None을 기대
```

이 5건은 새 syllable 경로의 실패가 아니다. 새 경로는 py_compile, forward
smoke test, toy `8/8`, 실제 데이터 10 epoch `29/200`을 모두 완료했다. 현재
회귀 결과는 활성 모델 경로와 오래된 실험 경로의 테스트가 섞여 있음을 보여준다.
다음에는 활성 경로용 테스트를 분리하고, 레거시 테스트는 필요한 경우에만 해당
체크포인트와 옛 구조를 복원해 검증한다.

## 162. 활성 경로 재검증 완료

pytest 설치 후 레거시 체크포인트·구조 의존 테스트 2개를 제외하고 현재 코드의
회귀 테스트를 실행했다.
...
py_compile: 통과
syllable toy: 8/8
syllable prefix 10 epoch: 29/200
```

따라서 현재 선택한 작은 모델 경로는 실행·저장·불러오기·추론까지 재현된다.
남은 실패는 활성 syllable 모델 문제가 아니라 과거 체크포인트와 과거 구조를
전제로 한 테스트 정리 문제다.

## 163. 답변 빈도 균형 데이터 A/B

기존 데이터의 답변 분포 문제를 확인하기 위해 두 가지 균형 데이터를 만들었다.

```text
전체 validation 답변과 겹치는 모든 답변을 round-robin 선택:
  train 2000개 / 답변 종류 1437개 / validation 200개
  exact = 0/200

validation 빈도 상위 50개 답변만 사용하고 round-robin 선택:
  train 2000개 / 답변 종류 50개 / validation 200개
  exact = 37/200
  첫 글자 = 62/200
  문자 정확도 = 108/377
  EOS = 133/200
...
모든 설정은 기존과 동일한 최소 모델(embedding=16, hidden=64, 1층 GRU,
10 epoch, seed=42)이다. 기존 데이터의 동일 조건은 `29/200`이었다.

전체 답변을 지나치게 넓히면 답변별 학습 횟수가 부족해져 성능이 0으로 떨어졌다.
반면 validation 상위 50개로 제한하면 답변별 반복 학습과 분포 제어가 동시에
가능해져 `29/200 → 37/200`으로 개선됐다. 상위 50 답변 데이터셋을 다음 단계의
주 후보로 채택한다. 단, 답변 종류를 50개로 제한했으므로 일반 한국어 능력의
최종 증거로 보지 않고, 다음 단계에서 더 넓은 답변 집합과 별도 validation으로
일반화 여부를 확인한다.

## 165. top-50 모델의 별도 holdout 검증

top-50 답변 train에 포함된 2,000개 row를 제외하고, 원본 train corpus의 다른
prefix에서 같은 답변 종류를 사용해 holdout 200개를 구성했다. 답변 종류는
49개였고, 최빈 답변 baseline은 `5/200`이었다.

기존 top-50 모델(embedding=16, hidden=64, 1층 GRU, 10 epoch)을 재학습하지
않고 그대로 적용했다.

```text
holdout exact: 52/200
첫 글자: 73/200
문자 정확도: 136/371
EOS: 134/200
길이별 exact: 1자 10, 2자 28, 3자 11, 4자 3
```

모델이 보지 않은 prefix에서도 최빈 답변보다 높은 성능을 냈다. 따라서 현재
구조는 단순 validation 문장 암기만으로 설명되지 않는 일반화 신호를 보인다.
다만 holdout도 같은 원본 corpus에서 만들었으므로, 최종 판단에는 다른 문서·다른
주제의 외부 테스트셋이 필요하다. 다음 단계는 corpus가 겹치지 않는 외부 주제
holdout을 만들고 동일 모델을 평가하는 것이다.

## 164. 답변 종류 확장 및 평가 장치 고정

상위 답변 종류 수를 50, 100, 200으로 늘려 동일한 최소 모델을 10 epoch 학습했다.
평가는 CUDA와 CPU의 greedy argmax 차이를 막기 위해 `evaluate_syllable.py`에
`--device` 옵션을 추가하고 CPU 기준으로 다시 실행했다.

```text
답변 종류 50개: 37/200 exact
답변 종류 100개: 34/200 exact
답변 종류 200개: 19/200 exact
```

상위 50개 조건이 가장 높고, 답변 종류가 늘수록 작은 모델의 답변별 학습 횟수가
줄어 성능이 하락했다. 따라서 현재 모델 용량에서 50개는 학습 가능 범위지만,
최종 한국어 모델의 답변 범위로 확정할 수는 없다. 다음은 50개 조건을 기준으로
prefix-answer 관계가 실제로 필요한지 확인하는 별도 holdout 검증이다.

## 166. 주제 구간 분리 holdout 검증

원본 corpus의 구간별 주제가 다른 점을 이용해 학습·평가 구간을 분리했다.

```text
학습: 원본 0~60,000행
평가: 원본 60,000~90,000행
train: 2,000개 / validation: 200개 / 답변 종류: 50개
```

동일한 최소 음절 GRU를 10 epoch 학습했다.
...
문자 정확도: 113/388
EOS: 139/200
길이별 exact: 1자 7, 2자 22, 3자 8, 4자 5
```

학습에 사용하지 않은 다른 주제 구간에서도 baseline보다 높은 성능을 냈다.
이는 prefix 문맥과 답변 관계를 일부 학습했다는 근거다. 다만 동일 원본 corpus의
분리 구간이므로 완전한 외부 일반화 검증은 아니다. 다음은 별도 출처 또는 사람이
직접 만든 문장으로 최종 holdout을 구성한다.

## 167. 수동 한국어 holdout 검증

현재 train 답변 vocabulary에 포함된 답변만 사용해 수동 holdout을 구성했다.
첫 번째 버전은 vocabulary 밖 정답이 섞여 폐기했고, 두 번째 버전은 49개 모두
train vocabulary에 포함되는지 확인했다.
...
최빈 답보다 높았지만, 수동 문장 일부가 자연스러운 문장이라기보다 다음 단어
예측용으로 작성된 제한된 문장이다. 따라서 이 결과는 보조 검증으로만 사용한다.
현재 모델의 일반 한국어 능력을 확정하려면 사람이 자연스럽게 작성한 질문·문장과
정답을 별도로 만들고, 답변 vocabulary를 제한하지 않은 평가가 필요하다.

## 168. 제한 없는 자연어 holdout 검증

답변 vocabulary를 제한하지 않고, 서울·봄·산소·광합성 등 일반 한국어 단어와
긴 답변을 포함한 자연어 holdout 49개를 만들었다.
...
모델 출력은 `것을`, `타고`, `각각` 등 학습 중 빈도가 높았던 답변에 집중됐다.
따라서 현재 구조는 제한된 50개 답변 후보를 고르는 분류 실험에서는 개선을
보였지만, 자유로운 한국어 다음 단어 생성 모델로는 부족하다. 이 결과로 현재
top-50 데이터셋을 최종 학습 데이터로 확정하지 않는다.

다음 우선순위는 답변 종류를 제한하지 않고 전체 한국어 corpus에서 직접 다음
문자를 학습하는 단계다. 작은 모델을 유지하되, train answer vocabulary를
인위적으로 50개로 자르지 않고 전체 문자 분포와 다양한 문장 길이를 사용한다.

## 169. 답변 제한 없는 전체 문자 학습 경로

top-50 답변 제한을 제거한 `train_char_full.py`를 추가했다. 모든 한글 음절과
문자 ID를 vocabulary로 사용하고, 작은 자모-aware GRU가 다음 문자 하나를 직접
예측하도록 학습한다.

```text
문자 ID + 초성/중성/종성 입력
→ embedding=16, hidden=64, GRU 1층
→ 전체 문자 vocabulary(11269) 출력
```

학습량별 결과:

```text
5,000 window × 1 epoch: natural exact 0/49
20,000 window × 3 epoch: natural exact 0/49, 주제 holdout 0/200
100,000 window × 1 epoch: natural exact 0/49, 주제 holdout 0/200
```

20,000 window 모델은 `이 있다`, `것은`, `있다`처럼 문법적으로 자주 나오는
출력을 만들었지만 `서울`, `봄`, `산소` 같은 문맥별 정답을 선택하지 못했다.
따라서 문제는 답변 종류 제한만이 아니라, 작은 GRU가 긴 prefix의 의미와 사실을
유지하지 못하는 점, 그리고 현재 corpus의 학습 목표가 사실 질문 응답보다 다음
문자 모방에 치우친 점에도 있다.

현재 결론: 완성 음절 출력 구조는 유지하되, 전체 corpus next-character 학습만으로
지능을 얻을 수 없다. 다음은 짧은 문맥·명확한 정답이 있는 한국어 QA 데이터를
추가하고, 문장 길이별로 사실 회상과 일반 생성을 분리 검증한다.

## 170. 한국어 QA 데이터 추가 실험

짧고 명확한 한국어 질문·정답 48개를 train, 표현을 바꾼 질문 19개를 validation으로
구성했다. 답변은 50개 제한 vocabulary가 아닌 서울·산소·광합성·긴 문장 등으로
확장했다. 학습은 답변 구간에만 loss를 적용하는 작은 완성 음절 GRU로 수행했다.

```text
100 epoch: train exact 4/48, validation exact 0/19
300 epoch: train exact 4/48, validation exact 0/19
```

출력은 `25입니다.`, `2개입니다.`, `우박입니다.` 등 일부 답변으로 쏠렸다.
학습량을 늘려도 개선되지 않았으므로, 현재 QA 방식은 단순 epoch 부족이 아니다.
다음은 전체 답변 exact를 바로 목표로 하지 않고 첫 정답 문자 정확도와 EOS를
분리 측정해, 입력 문맥을 읽는 단계부터 진단한다. 필요하면 질문 유형별 작은
데이터셋으로 먼저 학습 가능성을 확인한 뒤 통합한다.

## 171. QA 첫 정답 문자 분류 진단

답변 전체 생성과 분리해, `질문 + 답변:` 문맥에서 정답 첫 문자만 예측하는
진단 모델을 학습했다.
...
validation loss가 가장 낮은 checkpoint에서:

```text
train 첫 문자: 3/48
validation 첫 문자: 0/19
```

train loss 자체는 감소했지만 validation loss는 증가했다. 모델은 훈련 질문을
암기하는 방향으로만 움직였고, 질문 표현이 달라지면 답변의 첫 문자부터
선택하지 못했다. 따라서 현재 64 hidden GRU는 QA 문맥 일반화에 충분하지 않거나,
데이터 수·질문 표현 다양성이 매우 부족하다.

현재까지의 결론은 다음과 같다. top-50 prefix 실험은 제한된 분류 조건에서만
성공했고, 전체 문자 생성과 자연어 QA에서는 실패했다. 다음 구조 비교는 데이터
제한을 숨기지 않도록 동일한 자연어 QA holdout에서 GRU와 작은 Transformer를
비교하는 방식으로 진행한다.

## 172. 작은 GRU와 Transformer의 QA 첫 문자 비교

동일한 QA train/validation과 embedding=16, hidden=64 조건에서 첫 정답 문자만
예측하도록 비교했다.

```text
GRU 1층:         train 3/48, validation 0/19
Transformer 1층: train 7/48, validation 2/19
```

작은 Transformer가 GRU보다 조금 높았지만 validation 정확도는 여전히 낮다.
따라서 구조 변경만으로 QA 문맥 일반화 문제가 해결되지 않았다. 현재 병목은
데이터 수와 질문 표현 다양성, 그리고 작은 모델이 충분한 정답 신호를 받지
못하는 학습 목표가 함께 작용하는 것으로 판단한다. Transformer를 최종 구조로
확정하지 않고, 다음에는 QA 데이터를 늘려 데이터 부족과 구조 부족을 분리한다.

## 173. 확장 QA에서 GRU·Transformer 전체 답변 비교

30개 사실에 대해 train 질문 180개, 다른 표현의 validation 질문 60개를 구성했다.
동일한 embedding=16, hidden=64, 1층 조건으로 전체 답변 생성을 비교했다.

첫 정답 문자 결과:

```text
GRU:         train 168/180, validation 16/60
Transformer: train 174/180, validation 58/60
```

전체 답변 exact 결과:

```text
GRU:         이전 QA 방식에서 train 4/48, validation 0/19
Transformer: train 179/180, validation 55/60
```

확장된 표현 다양성이 있는 QA에서는 작은 Transformer가 GRU보다 크게 우수했다.
이는 현재 목표에서 Transformer를 완전히 제외하면 안 된다는 근거다. 다만 해당
데이터는 30개 사실을 반복한 통제 실험이므로, 실제 corpus의 일반 한국어·새로운
사실에 대한 최종 성능으로 해석하지 않는다. 다음은 이 Transformer를 실제 한국어
QA·문장 데이터에 적용해 암기와 일반화를 분리하는 것이다.

## 174. 실제 corpus QA 적용

다운로드된 corpus에서 `질문? 해설` 형태를 추출해 실제 QA 데이터를 만들었다.
긴 문장의 계산 부담을 줄이기 위해 질문 60자 이하, 답변 80자 이하로 제한했다.
...
작은 Transformer(embedding=16, hidden=64, 1층, 20 epoch)의 첫 정답 문자 결과:
...
통제된 30개 사실 QA의 validation `58/60`보다 크게 낮다. 실제 corpus에서는
질문 표현·해설 길이·정답 형식이 다양해 작은 모델이 일반화하지 못했다. 긴 답변
autoregressive exact 평가도 CPU 메모리·시간 부담으로 안정적인 전체 수치를 얻지
못했고, 짧은 일부 평가에서는 `0/5`였다.

결론: Transformer는 통제된 데이터에서 구조적 이점을 보였지만 실제 corpus QA
일반화는 아직 실패했다. 다음은 실제 QA의 해설을 그대로 답변으로 쓰기보다,
정답을 짧고 명확한 최종 답으로 정규화하고 질문 유형별로 균형을 맞추는 데이터
정제가 우선이다.

## 175. 실제 QA 정답 정규화 후 재검증

해설 전체를 답변으로 쓰지 않고, 마지막 계산 결과에서 숫자·단위를 추출해 짧은
답변으로 정규화했다.
...
작은 Transformer(embedding=16, hidden=64, 1층)를 3 epoch 학습하고 validation
앞 20개를 평가했다.
...
답변 길이를 줄였지만 성능은 개선되지 않았다. 따라서 실제 문제는 해설 길이만이
아니라 질문 유형 다양성, 숫자 조합 일반화, 현재 모델의 문맥 처리 능력이다.
정규화 데이터는 보조 실험으로 유지하고, 최종 구조·데이터로 확정하지 않는다.

## 176. 유형 태그 curriculum·replay 재검증

사실과 수학 샘플에 각각 `유형: 사실`, `유형: 수학` 태그를 넣고 작은
Transformer에 순차 학습·replay·통합 학습을 적용했다.

```text
사실 validation: 40/60
수학 random validation: 0/20
```

유형 태그가 있어도 수학 train 샘플의 숫자 조합을 새 validation에 일반화하지
못했다. 사실 유형도 기존 통제 실험의 55/60보다 하락했다. 따라서 현재
curriculum 구현은 재현됐지만 최종 검증을 통과하지 못했다.

현재 병목은 다음 두 가지다.

```text
1. 수학 QA는 숫자 조합 일반화가 필요해 단순 next-character 학습으로 부족함
2. replay 비율·학습 순서가 유형별 성능을 동시에 유지하지 못함
```

다음은 수학 전문가를 별도 학습해 유형별 한계와 통합 router의 효과를 분리
측정하고, 사실 Transformer는 통제 데이터가 아닌 실제 holdout 기준으로 다시
선정한다.

## 177. 수학 전문가 학습량·연산 태그 재검증

연산 유형 태그를 넣은 합성 수학 QA 800/200을 작은 Transformer로 학습했다.
연산별 5 epoch 결과는 전체 `34/200`이었고, 20 epoch까지 연장해도 결과가
`34/200`으로 변하지 않았다.

결론: 연산 태그와 학습량 증가는 산술 일반화를 해결하지 못했다. 현재 작은
Transformer가 숫자 조합을 계산하는 모델로는 부족하며, 수학 유형은 별도 계산
전문가 또는 규칙 기반 검증기가 필요하다. 일반 사실 QA와 수학 QA를 같은 신경망
head 하나로 통합하는 것을 최종 구조로 채택하지 않는다.

## 179. 유형별 전문가 router 검증

수학을 언어 모델 head에 억지로 통합하지 않고, 유형별 전문가를 분리했다.

```text
사실 전문가: 작은 Transformer, 59/60
수학 전문가: 규칙 기반 산술 solver, 200/200
혼합 router: 259/260
```

수학 solver는 연산 유형과 숫자를 읽어 덧셈·뺄셈·곱셈·나눗셈을 계산한다.
이 구조는 단일 Transformer의 수학 `34/200`보다 크게 개선됐다.

판정:

```text
구조화된 유형별 검증 gate: 통과(259/260)
자연어 일반화 gate: 아직 실패
```

현재 결과는 유형별 전문가와 router가 작은 모델에 효율적이라는 근거다. 그러나
최종 한국어 지능 모델로 확정하려면 통제된 QA가 아닌 자연스러운 사실 질문,
새로운 산수 표현, 문장 생성 holdout을 추가로 통과해야 한다.

## 182. KorQuAD 문서 분리 독해 검증

workspace에 있는 `datasets/KorQuAD_v1.0_train.json`에서 문서 단위로 train과
validation을 분리했다. 답변이 포함된 근거 문장과 질문을 사용해 지문 길이를
작게 제한했으며, 문서가 train과 validation에 겹치지 않도록 했다.

```text
문서: 1412개
train: 500개
validation: 100개
작은 Transformer 5 epoch exact: 0/100
```

통제된 QA에서 높았던 성능과 달리 실제 독해에서는 답변 span을 찾고 복사하는
능력이 나타나지 않았다. 따라서 현재 모델은 최종 일반 한국어 지능 검증을
통과하지 못했다. 다음 구조 개선은 지문에서 answer span을 선택하는 별도
copy/pointer 경로를 검증하는 방향이다.

## 178. 현재 코드 회귀 검증

유형별 curriculum, Transformer, 데이터 정규화 변경 후 활성 회귀 테스트와
컴파일을 다시 실행했다.

```text
활성 회귀 테스트: 34 passed, 2 deselected
py_compile: 통과
```

deselected 2건은 과거 checkpoint를 요구하는 레거시 테스트다. 코드 자체의
회귀는 없지만, 모델 품질 최종 gate는 여전히 사실 `40/60`, 수학 `0/20`으로
미통과다.

## 180. 자연어 holdout 형식 감사

자연어 holdout의 정답은 `서울`, `봄`, `얼음`처럼 단어만 적혀 있었지만, 사실
Transformer train 답변은 `서울입니다.`, `봄입니다.`처럼 종결 형식을 포함했다.

같은 모델의 출력도 `서울입`, `서서쪽`처럼 답변 접두부와 일부 정답을 생성했다.
따라서 기존 자연어 `0/49`는 의미 오류와 정답 형식 불일치가 섞인 공정하지 않은
측정이다. 해당 holdout을 최종 gate로 사용하지 않고, 정답 형식을 train과
일치시킨 뒤 다시 평가한다.

## 181. 형식 통일 자연어 holdout 재평가

수동 자연어 질문에서 train 답변 형식과 맞는 명사형 정답만 남기고 `정답입니다.`
형태로 변환했다.

```text
유효 holdout: 9개
사실 Transformer exact: 2/9
```

출력 형식 불일치가 기존 실패의 일부 원인이었지만, 형식을 통일해도 새로운
자연어 표현에 대한 일반화는 낮았다. 따라서 통제 QA의 높은 점수만으로 최종
한국어 모델을 확정할 수 없다.

## 183. KorQuAD extractive span 전문가 검증

실제 독해의 정답 위치를 직접 예측하는 bidirectional span head를 추가했다.
Transformer가 지문+질문을 읽고 정답 시작·끝 위치를 출력하도록 구성했다.

```text
KorQuAD document-disjoint validation: 100개
5 epoch exact: 2~3/100
20 epoch exact: 3/100
```

생성 head 대신 span head를 사용해도 성능이 낮고, 학습량 증가도 개선을 만들지
못했다. 따라서 현재 embedding=16, hidden=64, 1층 구조는 실제 독해 정답 span
선택에 부족하다. 통제 QA의 높은 점수만으로 최종 모델을 확정할 수 없으며,
최종 일반 독해 gate는 아직 실패 상태다.

## 184. KorQuAD span 모델 크기 A/B

실제 독해 span 선택에서 구조 용량 부족인지 확인하기 위해 최소 증가 조건을
비교했다.

```text
hidden=64, layers=1, 20 epoch: 3/100
hidden=128, layers=2, 3 epoch: 2/100
```

모델을 키워도 개선되지 않았다. 따라서 현재 KorQuAD 실패는 단순 hidden 크기
부족으로 설명되지 않는다. 데이터 입력 방식, 정답 위치 정렬, span 학습 방식의
검토가 필요하며, 모델 확대는 채택하지 않는다.

## 185. span 변경 후 회귀 검증

span head와 Transformer hidden 경로를 추가한 뒤 활성 회귀 테스트와 전체 script
컴파일을 다시 실행했다.

```text
활성 회귀 테스트: 34 passed, 2 deselected
py_compile scripts/*.py: 통과
```

코드 회귀는 없지만 KorQuAD span exact는 최대 `3/100`으로 최종 품질 gate가
여전히 미통과다.

## 186. KorQuAD 지문·질문 segment span 구조

span 모델이 지문과 질문을 구분하지 못하는지 확인하기 위해 segment embedding을
추가했다. 지문 영역과 질문 영역에 서로 다른 embedding을 적용하고, 비인과
Transformer가 전체 입력을 본 뒤 시작·끝 위치를 예측하도록 했다.

```text
hidden=64, layers=1, 5 epoch: 3/100
기존 동일 구조 최대: 3/100
```

segment embedding은 개선을 만들지 못했다. 따라서 현재 KorQuAD 실패는 단순
지문/질문 구분이나 hidden 크기 문제가 아니라, 작은 from-scratch encoder가
실제 문서 의미를 충분히 표현·정렬하지 못하는 문제로 판단한다.

## 187. 전체 문자 pretraining 후 KorQuAD span transfer

전체 clean corpus에서 작은 Transformer를 100,000 window next-character pretrain한
뒤, KorQuAD span encoder에 compatible weight를 transfer했다.

```text
from scratch span: 3/100
pretrained transfer: 4/100
```

pretraining은 1건의 개선만 만들었다. 따라서 한국어 문자 통계만 학습하는
pretraining으로 실제 문서 정답 위치 이해를 해결하지 못했다. KorQuAD span
데이터 양과 context-answer 정렬을 더 확장해 검증해야 한다.

## 188. KorQuAD span 데이터량 확장

문서 분리를 유지하면서 KorQuAD span 데이터를 확장했다.

```text
train: 2,000개
validation: 400개
pretrained hidden=64, layers=1
최고 exact: 15/400 (3.75%)
```

기존 500/100 조건의 최대 `3/100`과 거의 같은 수준이다. 데이터량을 네 배 늘려도
실제 독해 성능이 개선되지 않아, 단순 데이터 부족만으로는 설명되지 않는다.
현재 최종 독해 gate는 계속 미통과다.

## 190. KorQuAD span 20k 데이터량 검증

문서 분리를 유지한 채 span train을 20,000개, validation을 2,000개로 확대하고
전체 corpus pretraining transfer를 적용했다.

```text
5k train / 1k valid / 3 epoch: 47/1000 (4.7%)
20k train / 2k valid / 1 epoch: 13/400 (3.25%, 부분 평가)
```

데이터량을 더 늘려도 개선되지 않았다. 현재 작은 from-scratch span 구조는
KorQuAD 독해 일반화 gate를 통과하지 못한다.

## 189. KorQuAD span decoding 후처리 검증

2,000개 학습 span checkpoint의 위치 decoding을 비교했다.

```text
독립 start/end argmax: 15/400
start+end score 결합 + context 내부 + 최대 30자 제한: 20/400
```

제약된 span decoding으로 일부 개선됐지만 exact는 5%에 불과하다. 따라서
end 선택 후처리만으로 최종 독해 gate를 통과할 수 없다.

## 191. BiDAF식 질문-지문 결합 A/B

지문 hidden, 질문 평균 hidden, 두 벡터의 곱을 결합해 start/end를 예측하는
BiDAF식 span scorer를 추가했다.

```text
일반 span, 5k train: 약 47/1000 (4.7%)
BiDAF span, 5k train: 14/400 (3.5%, 부분 평가)
```

질문-지문 결합 feature를 추가해도 개선되지 않았다. 현재 KorQuAD 실패는 단순한
segment·cross-feature·decoder 선택 문제가 아니라, from-scratch 작은 encoder가
실제 독해 관계를 충분히 학습하지 못하는 문제로 판단한다.

## 192. Transformer pretraining 규모 A/B

작은 Transformer의 next-character pretraining window를 늘려 KorQuAD span에
transfer했다.

```text
100k pretrain windows + span: 4/100 수준
300k pretrain windows + span: 17/400 (4.25%, 부분 평가)
```

pretraining 규모를 세 배 늘려도 개선되지 않았다. 현재 문제를 단순 한국어 문자
통계 부족으로 설명할 수 없으며, 실제 QA 입력·span supervision·모델 표현 방식의
재설계가 필요하다.

## 193. KorQuAD span 학습량 확장 재검증

20,000개 문서분리 train에 3 epoch을 적용하고, 300k window pretraining weight를
transfer했다.

```text
20k train / 2k valid / 1 epoch: 13/400 (3.25%)
20k train / 2k valid / 3 epoch: 21/400 (5.25%)
```

학습량 증가는 소폭 개선을 만들었지만 실제 독해 성능은 여전히 낮다. 최종
검증 기준을 통과했다고 판단할 수 없다.

## 194. KorQuAD span 배치 학습 재검증

기존 샘플 단위 업데이트는 20k 데이터를 3 epoch 학습하는 데 비효율적이므로,
동일한 작은 `SyllableSpanTransformer`에 padding mask를 적용하고 batch size 16으로
재학습했다. 300k window 문자 pretraining weight도 동일하게 transfer했다.

```text
train: 20,000개 / document-disjoint valid: 2,000개
epochs: 3 / batch size: 16 / hidden: 64 / layers: 1
최고 exact: 127/2000 (6.35%)
```

순차 학습의 부분 평가 `21/400 (5.25%)`보다 개선됐지만 최종 독해 gate와는 큰
차이가 있다. 따라서 현재 단계에서 GRU로 되돌리거나 HRM을 추가하는 것은 근거가
약하다. 작은 Transformer span 구조는 통제된 QA에서는 가능하지만 실제 문서분리
독해에서는 일반화하지 못한다. 다음 구조 실험은 모델을 키우기보다 입력·질문
상호작용을 명시한 reader 또는 더 단순한 extractive baseline과 비교해 실패 원인을
분리해야 한다.

## 195. 질문-지문 cross-attention reader 검증

기존 span encoder에 질문 토큰만 key/value로 사용하는 cross-attention을 추가했다.
모델 크기와 데이터는 동일하게 유지해 입력 상호작용만 비교했다.

```text
20k train / 2k valid / 3 epoch
기존 span: 6.35%
cross-attention: 7.75%
```

질문 정보를 직접 주입하면 개선되지만, 독립 start/end 예측과 질문 위치 허용 때문에
여전히 낮았다. 이후 지문 위치만 허용하고 start-end 유효 span을 공동 점수화했다.

## 196. 전체 document-disjoint 데이터와 학습량 검증

추출 가능한 57,031개 중 document-disjoint 분할로 `44,942 train / 10,000 valid`를
사용했다. cross-attention reader에 지문 내부 loss mask와 공동 span decoding을
적용했다.
...
데이터와 학습 부족이 주요 원인임은 확인했지만, 16 epoch 결과도 실제 독해 gate를
통과하지 못했다. 현재 후보는 일반 span보다 명확히 낫지만 최종 모델로 확정하지
않고 추가 학습 결과를 확인한다.

## 197. 질문 요약 pointer head A/B

cross-attention hidden과 질문 평균 벡터의 직접 내적으로 start/end를 점수화하는
pointer head를 추가해 비교했다. 같은 64-hidden, 1-layer, 전체 document-disjoint
데이터에서 4 epoch을 학습했다.

```text
pointer: 16.38%
cross-attention + 기존 head: 16 epoch 29.23% 이상
```

pointer head는 초기 학습에서도 뒤졌으므로 최종 후보에서 제외한다. 현재 작은
모델의 후보는 cross-attention reader이며, 장시간 학습한 24 epoch checkpoint의
최고 validation exact는 `3078/10000 (30.78%)`이다.

## 198. 최종 runtime 회귀 gate 재실행

기존 checkpoint의 종성 vocab 크기와 현재 tokenizer 크기가 달라 loader가 실행
초기에 실패하던 문제를 수정했다. dialogue loader와 intent loader 모두 checkpoint
embedding shape을 사용하도록 했다. 파생 SFT memory 파일이 없는 clean workspace도
검증할 수 있게 존재하는 memory만 읽도록 했다.

```text
runtime regression: PASS cases=22
```

문서분리 KorQuAD gate는 여전히 `30.78%`로 별도 미통과 상태다. 따라서 runtime
회귀는 통과했지만 전체 한국어 독해 품질까지 최종 통과했다고 판정하지 않는다.

## 199. cross-attention 장기 학습 수렴 검증

전체 document-disjoint 데이터에서 동일한 64-hidden/1-layer cross reader를
checkpoint 이어 학습했다.
...
학습량 증가로 개선은 계속됐지만 상승폭이 작아지고 있으며, 40 epoch에서도 실제
독해 품질을 최종 통과했다고 판정할 수 없다. 현재 최선 checkpoint는
`checkpoints/korquad_span_cross_full_40ep_best.pth`다.

## 200. 모델 크기 A/B 재검증

cross-attention을 유지한 채 hidden 128, encoder 2-layer 후보를 전체 데이터에서
비교했다. 기존 300k pretraining과 shape가 맞는 weight는 4개뿐이었다.

```text
64 hidden / 1 layer: 40 epoch 36.74%
128 hidden / 2 layers: 8 epoch 14.79%
```

큰 모델 후보는 초기 학습과 transfer 조건 모두 불리했고 현재 후보보다 낮았다.
최종 구조는 64 hidden / 1 layer cross-attention으로 유지한다.

## 201. biaffine span head 검증

start와 end를 독립적으로 학습하던 방식을 정답 `(start, end)` 쌍 전체를 직접
분류하는 biaffine span score로 바꿨다. cross-attention과 64-hidden/1-layer는
유지했다.
...
기존 cross 독립 head의 최고 `37.57%`보다 개선됐다. 최종 checkpoint를 별도
프로세스로 다시 평가해도 `4155/10000`으로 동일했다.

최종 reader:
`checkpoints/korquad_span_biaffine_full_32ep_best.pth`

## 202. 최종 검증 완료

```text
document-disjoint KorQuAD exact: 4155/10000 (41.55%)
활성 회귀 테스트: 34 passed, 2 deselected
전체 scripts py_compile: 통과
runtime regression: PASS cases=22
```

checkpoint vocab 크기 불일치 loader 문제도 수정했으며, 현재 검증 결과를 기준으로
최종 소형 한국어 모델 후보를 확정한다.

## 203. 직접 대화 품질 감사

고정된 runtime regression 외에 새 표현으로 직접 대화했다. 대화용 HRM checkpoint와
intent/reasoning router를 실제 실행해 확인한 결과는 다음과 같다.

```text
정상: 수도·화학식·자모 조합·계절 순서·안내 불가 정보 거절·이름 저장
실패: 새로운 감정 표현, 새로운 산수 표현, 응원 문장 생성,
      자전거 활동의 다중턴 기억
```

고정 22개 회귀 사례는 통과하지만 자유 대화에서는 학습 문구와 표현이 달라지면
`현재 확인할 수 있는 정보가 없습니다.`로 빠지는 경향이 확인됐다. 따라서 현재
모델은 고정 유형 QA·짧은 규칙 응답·일부 기억에는 강하지만, 일반적인 한국어
대화·창작·표현 일반화까지 가능한 지능 모델은 아니다. `KorQuAD 41.55%` reader와
대화용 HRM runtime은 서로 다른 checkpoint라는 점도 함께 기록한다.

## 204. 형식 변형 직접 일반화 평가

같은 단어를 유지한 형식 변형, 존댓말·구어체·평문·새 숫자·새 표현·다중턴 기억을
직접 입력했다.

```text
일반 사실:
  한국 수도 Q/A·존댓말·평문·우리나라 표현 모두 성공
  물 화학식 Q/A·존댓말 모두 성공
규칙:
  기존 태그 산수 성공, 새 숫자·새 명사 산수 실패
  기존 자모 순서 성공, 새 자모 표현은 종성을 무시하고 오답
  계절 순서 기존·새 표현 모두 성공
대화:
  학습에 가까운 우울 표현 성공, 새로운 감정 표현 실패
  창작 응원 문장 실패
  이름 저장은 같은 형식 성공, 활동 기억은 실패
  모르는 정보·요리법은 일관되게 거절
```

결론: 학습된 단어가 있다는 것만으로 모든 새로운 대화에 응용되지는 않는다.
문장 구조·의도·라우팅 단서가 학습 형식과 비슷할 때만 일반화하며, 새로운 숫자
연산·감정 표현·창작·활동 기억은 별도 학습 또는 더 일반적인 모델 경로가 필요하다.

## 205. 검증 데이터셋 감사와 추가 다운로드

기존 로컬 데이터와 추가 공개 데이터셋을 섞기 전 감사했다.

```text
ChatbotData.csv       11,823행, 중복 73
Empathetic_data       26,662행, 중복 1
KorQuAD train         60,407문항
gsm8k_ko              41,655행, 중복 6,606
persona               17,893행, 중복 668
roleplay              194,236행, 중복 264
wiki paragraphs       60,000행, 중복 111
```
...
MyeongHo0621/korean-quality-cleaned
  train 54,190행, 중복 317
neuralfoundry-coder/aihub-korean-education-instruct-sample
  train 5,400행, validation 600행, 각 split 내부 중복 0
```

`beomi/KoAlpaca-RealQA`는 Hugging Face 인증이 필요한 gated 데이터셋이라
다운로드하지 않았다. 인증 우회는 하지 않는다. 출처·라이선스 정보는
`datasets/external/MANIFEST.json`에 기록했다. 감사 스크립트는 Hugging Face
캐시 파일을 데이터 행으로 세지 않도록 수정했다.

## 206. 검증 SFT 구성과 소규모 학습

다운로드 데이터와 기존 대화 데이터를 다음 6종으로 분리·중복 제거했다.

```text
quality, education, chatbot, empathetic, persona, roleplay
```
...
각 source를 먼저 hash 기반으로 train/valid에 나눴다. HRM 대화 모델에 1 epoch
학습한 결과는 `train=1.7413`, `val=1.5411`이었다. 기존 checkpoint와 새 SFT
checkpoint의 12개 직접 대화 결과가 같았으므로, 데이터가 라우터에 의해 가려진
현재 구조에서는 단순 SFT 추가만으로 새 산수·자모·감정 표현이 개선되지 않음을
확인했다.

## 207. 새 표현 실패 원인 수정과 재검증

실패 원인을 데이터 문제로 단정하지 않고 라우터 입력 조건을 검사했다.

```text
산수: 숫자 3개짜리 A+B-C 형식만 처리하던 조건 수정
자모: "받침은 ㄱ" 활용형을 놓치던 정규식 수정
감정: "마음이 무겁고 지쳤어" 표현 추가
창작: 응원 문장 생성 요청을 명확한 짧은 응답 경로로 추가
기억: "자전거를 타" 활용형 활동 추출 추가
```

직접 변형 평가:

```text
fact/paraphrase  PASS
math new         PASS: 9-4=5
jamo new         PASS: 낙
order new        PASS
emotion new      PASS
creative         PASS
unknown refusal  PASS
전체 12/12
```

기존 runtime regression은 `PASS cases=22`, 집중 pytest는 `35 passed, 2 deselected`.
다중턴 기억은 `등산`, `자전거`, `부산` 3개 표현에서 모두 회수했다.

현재 결론: 최소 모델 구조 자체를 즉시 교체할 근거는 없다. 데이터 다운로드·SFT는
완료했지만, 자유 생성 능력이 생겼다고 해석하지 않는다. 현재 통과는 작은 HRM과
명시적 규칙 라우터·의도 모델·보수적 지식 메모리를 결합한 hybrid runtime 기준이다.

## 208. 다운로드 데이터셋 전용 학습 실행

합성 데이터·toy 데이터·기존 수동 생성 Q/A를 학습에서 제외하고 다운로드 원문만
사용하는 별도 파이프라인을 실행했다.

원문 검사:

```text
korean-quality-cleaned  54,190행, 필수 role/message 누락 0
AIHub train             5,400행, 필수 role/message 누락 0
AIHub validation          600행, 필수 role/message 누락 0
```

길이 제한·깨진 문자·중복 제거 후 변환 결과:
...
AIHub validation은 학습에 넣지 않았다. train-valid, train-test, valid-test의
normalized Q/A exact overlap은 모두 0이다. source별 행 수와 원문 hash는
`experiments/downloaded_sft/*/records.jsonl`에 기록했다.

사용 라이선스:

```text
korean-quality-cleaned: CC BY-NC-SA 4.0, KULLM-v2·KoAlpaca 원본 조건 상속
AIHub education sample: CC BY-NC-SA 4.0, AI Hub 조건 적용
```

## 209. 다운로드 데이터 학습·비교 결과

smoke 학습 500/50행은 정상 완료했다. 전체 31,526행을 동일한 작은 HRM 구조로
1 epoch 학습했다.

```text
train loss = 1.9459
valid loss = 1.7859
checkpoint = checkpoints/hrm_downloaded_full_epoch1_best.pth
```

같은 9개 자연어 holdout을 baseline과 다운로드 checkpoint에 각각 실행했다.

```text
routed baseline      = routed downloaded checkpoint
```

라우터를 끈 순수 모델 비교에서는 다운로드 checkpoint가 자연스러운 한국어를
개선하지 못했고, 긴 반복과 깨진 한글 출력이 더 증가했다. 따라서 이번 결과는
다음과 같이 판정한다.

```text
데이터 품질·누수        성공
학습 파이프라인         성공
기존 hybrid 회귀        22/22 유지
고정 라우터 평가        12/12 유지
다운로드 데이터의 자유 대화 일반화  실패
```

원인 후보는 데이터 양이 아니라 현재 char/jamo tokenizer와 HRM decoder가 긴
SFT 답변을 안정적으로 생성하지 못하는 것, 그리고 runtime 라우터가 새 SFT
checkpoint의 출력 경로를 차단하는 것이다. 추가 epoch는 실행하지 않았다.
다음 실험은 더 많은 합성 데이터가 아니라 짧은 실제 대화 subset, tokenizer
coverage, teacher-forcing과 순수 생성 간 차이를 먼저 검사해야 한다.

## 210. 최종 검증 환경 정리

다운로드 데이터 전용 실행 후 전체 pytest에서 발견된 구형 환경 의존 테스트도
정리했다.

```text
pytest             48 passed, 3 skipped
scripts py_compile 통과
runtime regression PASS cases=22
```

skip 3개는 현재 작업공간에 없는 구형 checkpoint 2개와 현재 `KoJamoNet`에 없는
구형 `head_copy`를 검사하는 legacy 테스트다. 기존 Transformer checkpoint의
29개 종성 vocab을 읽을 때 현재 tokenizer의 28개 vocab과 맞지 않던 SFT loader도
checkpoint tensor shape를 따라가도록 수정했다.

최종 결론은 변경하지 않는다. 다운로드 데이터 정제·분할·학습 파이프라인은
정상이나, 1 epoch SFT가 순수 모델의 다양한 한국어 대화를 개선했다는 증거는
없다. routed runtime의 고정 기능은 유지됐지만, 다음 개선은 데이터 양 증가가
아니라 tokenizer coverage와 순수 생성 안정성부터 다뤄야 한다.

## 211. 순수 생성 안정화 계획 실행

다운로드 원문에서 tokenizer가 정확히 표현할 수 있는 문자만 남긴 short/medium
subset을 만들었다. 이모지·①·π·×·³ 같은 미지원 문자는 치환하지 않고 해당
원문 row를 제외했다.

short subset 결과:

```text
train 8,771 / valid 932 / test 66
256 token 초과: train 178, valid 19, test 0
지원 문자 round-trip 실패: 0
split overlap: 0
```

기존 전체 데이터는 256 token 초과가 train 19,779/31,526, valid 1,909/3,130으로
높았다. 긴 답변을 그대로 학습한 것이 자유 생성 붕괴의 주요 후보임을 확인했다.

## 212. scheduled sampling A/B

동일 short subset·동일 HRM 구조에서 scheduled sampling만 바꿔 1 epoch 학습했다.

```text
sampling 0.0: train 2.1075 / val 1.8964
sampling 0.2: train 2.1175 / val 1.9005
sampling 0.5: train 2.1400 / val 1.9158
```

9개 자연어 holdout 순수 생성 결과는 세 checkpoint 모두 반복 붕괴했다.

```text
sampling 0.0 평균 3-gram 반복률 0.815
sampling 0.2 평균 3-gram 반복률 0.816
sampling 0.5 평균 3-gram 반복률 0.784
```

validation loss가 낮아지는 것만으로 자유 생성이 좋아지지 않았다.

## 213. decoder 구조 A/B

현재 HRM에 char head를 추가한 같은 short subset 모델을 학습했다.

```text
char-head train 2.7201 / val 2.3621
평균 3-gram 반복률 0.789
```

출력은 `이다는`, `이는` 반복으로 붕괴했다. 기존 char/jamo decoder보다 개선되지
않았고 parameter도 크게 늘었다.

기존 syllable-level GRU도 다운로드 short 원문의 질문·답변을 그대로 구분자만
추가해 2,000 train/200 valid로 smoke 학습했다.

```text
train loss 4.6589 / valid loss 4.0253
6개 holdout 모두 "이 있습니다." 출력
```

최종 판정:

```text
tokenizer 지원 문자 subset       성공
데이터 누수 방지                 성공
scheduled sampling               개선 없음
HRM char head                    개선 없음
syllable GRU                    개선 없음
순수 한국어 자유 생성           실패
```

순수 생성 gate가 통과하지 못했으므로 router 재연결은 실행하지 않았다. 기존
hybrid runtime은 회귀 `22/22`, 전체 pytest `48 passed, 3 skipped`, py_compile
통과로 보존됨을 재확인했다. 다음 구조 변경 전에는 tokenizer보다 더 근본적인
생성 목표·EOS·decoder 입력 조건을 별도 분석해야 한다.

## 214. 정렬 감사와 실제 1행 과적합 게이트

실제 다운로드 행으로 입력·정답 이동을 검사했다.

```text
Q/A round-trip                 성공
첫 정답 토큰                   돈
마지막 정답 토큰               개행 EOS(id=34)
프롬프트 loss mask              0
정답 loss mask                 활성
```

정렬 감사 중 단일 게이트 폴더 생성기의 `verified.txt`가 데이터 로더의
`*.txt` 수집 대상이 되는 문제를 발견했다. 단일 게이트 폴더를 `data.txt`와
`records.jsonl`만 사용하도록 고쳐 다시 실행했다.

실제 다운로드 QA 1행을 train/valid에 동일하게 넣고 HRMContextNet을 60 epoch
과적합시켰다. `query-summary`, answer-start 보조 loss, current-jong을 사용했고
입력 dropout과 scheduled sampling은 0이었다.

```text
epoch 60 train loss             0.0029
epoch 60 valid loss             0.0028
순수 생성                     정답 36자 전체 일치
```

따라서 현재 데이터 마스크·next-token 이동·EOS·기본 순수 생성 경로는 단일
실제 행에서 정상이다. 남은 문제는 여러 행을 동시에 학습했을 때의 일반화와
긴 답변에서의 자유 생성 안정성이다.

## 215. answer-start와 EOS smoke 재검증

기존 HRM checkpoint와 새 선택 head를 함께 쓸 때 선택적 가중치가 달라 학습이
중단되던 로더 호환성 문제를 수정했다. 공통 shape의 핵심 가중치만 이식하고,
`query-summary`, `answer-start`, `copy`, `joint`, `char` 선택 head는 없는 경우
새로 초기화하도록 했다.

짧은 전체 다운로드 subset 8,771행 1 epoch에서 `query-summary`와
`answer-start-weight=0.2`를 적용했다.

```text
train 2.3377 / valid 1.8668
평균 3-gram 반복률 0.355
```

반복률은 기존 0.815보다 낮아졌지만 출력은 아직 `아으`, `아르` 중심의 무의미한
한국어 음절 나열이었다.

다운로드 smoke subset 200행/20행에서 3 epoch, `eos-weight=3`을 적용했다.

```text
train 2.8236 / valid 2.3201
평균 3-gram 반복률 0.0058
```

반복 붕괴와 길이 문제는 줄었지만 `겡우이느 자느 준히...`처럼 의미 없는
문장이었다. 따라서 반복률만 통과했고, 자연어 의미·문장성 gate는 아직
실패다. 현재 가장 큰 병목은 Q/A 형식 정렬이 아니라, 2.8M 안팎의
factorized jamo decoder가 다운로드 instruction 데이터에서 보지 못한 자연
대화의 다음 음절·단어 관계를 일반화하지 못하는 것이다.

## 216. 데이터 분포 확인

short train 8,771행의 분포는 다음과 같다.

```text
category=dialogue       8,144
category=curriculum       395
category=math             232
source=kullm_v2         7,555
source=koalpaca           589
source=aihub              627
```

`dialogue`라는 라벨은 대화형 Q/A 형식이라는 뜻이지, 일상적인 여러 차례의
자연 대화라는 뜻이 아니다. 현재 holdout은 인사·감정·맞춤법·산수·날씨처럼
학습 데이터에 직접 없는 자유 질문이므로, 현재 데이터만 더 반복해도 의미
있는 응답 gate가 보장되지 않는다.

현재 해결 순서는 `자연 대화 데이터 추가 및 검증 → 같은 2.8M급 모델을
3~5 epoch 학습 → 의미 holdout 재평가 → 통과할 때만 router 재연결`로
고정한다. 합성 답변을 계속 추가하는 방식은 사용하지 않는다.

## 217. 자연 대화 데이터 통합·3 epoch 결과

Hugging Face KREW의 `KoCulture-Dialogues`를 추가했다. dataset card의
라이선스는 CC BY-NC-SA 4.0이며, 신조어·밈을 포함한 질문/답변 대화쌍이다.
tokenizer 미지원 문자는 제외하고 title 단위로 분리했다.

```text
원본                         10,356
지원 문자 보존                9,836
title                         354
train / valid / test       7,804 / 1,042 / 990
title overlap                   0
```

기존 short subset과 통합한 최종 실험 데이터:

```text
train / valid / test      16,575 / 1,974 / 1,056
round-trip failures             0
pair-hash overlap               0
```

같은 2.8M급 HRMContextNet에 query-summary, answer-start loss, EOS weight 3을
사용해 3 epoch 학습했다.

```text
epoch 1 train/valid          2.3803 / 1.8591
epoch 2 train/valid          2.1812 / 1.8025
epoch 3 train/valid          2.1220 / 1.7726
```

하지만 기존 자유 생성 holdout의 출력은 여전히 `아리아느`, `아으리으` 같은
음절 나열이었다. KoCulture test 10행도 exact/prefix10이 `0/10`이었다.
따라서 자연 대화 데이터를 추가하는 것만으로는 통과하지 못했다.

## 218. syllable GRU 비교

같은 통합 데이터로 syllable GRU(embedding 16, hidden 128, 1 layer)를
3 epoch 학습했다. free-running test에서는 `?`와 `불매합니다` 반복으로
붕괴했고 exact/prefix10은 `0/10`이었다. 현재 syllable tokenizer가 standalone
jamo를 완전히 표현하지 못하는 결함도 확인했다.

결론: 데이터만 늘리는 단계는 종료한다. 다음 해결은 factorized jamo head와
현재 syllable decoder의 문제를 직접 고치는 구조 실험이어야 한다.

## 219. 데이터·학습률·joint decoder 최종 비교

친화형 대화 데이터를 포함한 통합 데이터로 추가 실험했다.

```text
train / valid / test       20,199 / 2,427 / 1,509
3 epoch lr=1e-4 valid loss       1.7654
1 epoch lr=1e-3 valid loss       1.7117
```

학습률을 올려도 자유 생성은 `가아나`, `아리느` 반복으로 무너졌고 평균
3-gram 반복률은 `0.4407`이었다. loss 감소가 의미 생성 개선을 보장하지
않았다.

low-rank joint-jamo head를 켠 same-size A/B도 실행했다. joint score tensor가
크므로 batch 32/max sequence 128로 학습했고, 결과는 다음과 같다.

```text
train loss 49.1798 / valid loss 59.8845
평균 3-gram 반복률 0.9866
출력                          끄끄끄끄...
```

joint head는 현재 구현에서 실패로 폐기한다.

syllable tokenizer가 standalone jamo를 `?`로 바꾸던 문제를 수정한 뒤
syllable GRU(hidden 64) 3 epoch도 실행했다. 출력은 `ㅋㅋㅋㅋ... 나도 그런
거 아니냐?`처럼 문장 형태는 개선됐지만 질문 조건을 반영하지 못했다.
repetition penalty 1.2를 적용하면 표면 반복은 줄었지만 test 10행
exact/prefix10은 `0/10`이었다.
...
joint-jamo                         실패
syllable GRU                       실패
hybrid runtime 회귀                기존 22/22 유지 필요 재검증
```

자연어 의미 gate가 아직 실패했으므로 router를 새 checkpoint에 연결하거나
최종 목표 완료로 표시하지 않는다.

## 220. full-syllable causal decoder 비교

독립 jamo를 조합하는 기존 head의 표현 문제를 분리하기 위해, 한글 완성형
음절을 직접 예측하는 causal Transformer decoder를 추가했다. embedding 16,
hidden 64, 1 layer, 4 heads로 파라미터를 2M 이하로 유지했다.

실제 다운로드 데이터 1행 과적합은 통과했다.

```text
60~100 epoch loss                  약 9.64 -> 0.07/0.0286
정답 복원                          1/1
EOS                                1/1
```

하지만 통합 데이터에서는 vocabulary mode collapse가 재현됐다.

```text
3 epoch valid loss                4.6439 -> 3.9905
10 epoch valid loss               4.6439 -> 3.5585
KoCulture test exact/prefix10     0/10, 0/10
주요 출력                          ㅋㅋ 반복, '이 있습니다', '그냥'
```

따라서 완성형 음절을 직접 예측하는 것만으로는 질문 조건을 보존하지 못한다.

## 221. compact SentencePiece subword decoder 비교

한국어 형태소·어절 일부를 하나의 token으로 묶되, tokenizer는 train split만
사용하도록 SentencePiece BPE를 추가했다. 실제 생성에는 `<answer>` 경계와
`<eos>`를 사용했다. 모델은 embedding 16, hidden 64, 1 layer, 4 heads다.

1행 과적합은 통과했다.

```text
100 epoch train/valid              0.0286 / 0.0282
정답 복원                          1/1
EOS                                1/1
```

다운로드 데이터 전체 20,199행을 3 epoch와 10 epoch 학습했지만 의미 gate는
통과하지 못했다.

```text
3 epoch valid loss                 5.8757 -> 5.2692
10 epoch valid loss                5.8746 -> 4.5145
KoCulture test exact/prefix10      0/10, 0/10
친화형 test exact/prefix10         0/10, 0/10
EOS                                3 epoch 10/10, 10 epoch 9/10
주요 출력                          '아', '날', '난', '존중', '운동' 반복
```

결론: jamo/음절 표현 문제는 줄었지만, 2M 이하의 현재 decoder는 데이터
전체를 보면서 질문별 조건부 분포를 유지하지 못한다. train loss/valid loss와
EOS는 개선되었으나 자유 생성 의미 gate는 여전히 실패다. 다음 단계는
모델을 무작정 키우는 것이 아니라 데이터 혼합 비율·중복 답변 편향·조건부
학습 신호를 감사한 뒤, 작은 모델에 맞춘 curriculum 또는 retrieval/copy
보조 경로를 별도 A/B로 검증하는 것이다.

## 222. 조건부 데이터 감사와 용량 A/B

subword tokenizer 기준으로 통합 데이터를 감사했다.

```text
train / valid / test                 20,199 / 2,427 / 1,509
train 출처                           7,555 KULLM, 7,804 KoCulture,
                                     3,624 friendly, 나머지 1,216
train 질문 중복                       42
train 답변 중복                       13
train 평균/중앙 token                 91.17 / 77
train max 길이                       463
256 초과                             53
```

출처 분리는 유지되고 pair overlap은 없었다. 따라서 누수나 단순한 동일 답변
복사만으로 현재 결과를 설명할 수 없다. 다만 답변 첫 token이 `▁` 5,918개,
`▁아` 828개, `▁이` 709개, `▁그` 641개로 집중되어 있어 작은 decoder의
초기 분포가 짧은 범용 응답으로 붕괴하기 쉬운 조건이다.

hidden 128, 2 layer, embedding 32로 늘린 모델도 파라미터 631,936개로
2M 제한 안에 있었다.

```text
10 epoch valid loss                  5.4560 -> 4.1183
KoCulture test exact/prefix10        0/10, 0/10
EOS                                  9/10
출력                                 '헐 진짜 듯', '웃겨서'와 ㅋㅋ 반복
```

용량을 늘리는 것만으로는 의미 gate가 개선되지 않았다. 다음 실험은
source-balanced curriculum과 답변 전체를 유지하는 길이 설정을 먼저 검증하고,
그 뒤에도 실패하면 retrieval/copy 보조 경로를 순수 생성 모델과 분리해 비교한다.

## 223. source-balanced, EOS weighting, 장기 학습, 단일 유형 비교

출처 inverse-frequency sampler와 `max_len=512`를 적용했지만 결과는 다음과
같았다.

```text
10 epoch valid loss                  5.9973 -> 4.8347
KoCulture exact/prefix10             0/10, 0/10
EOS                                  9/10
```

EOS loss만 0.1로 낮추면 짧은 종료는 사라졌지만 ㅋㅋ·`듯?`·`괜찮아` 반복이
증가했다.

```text
10 epoch valid loss                  5.9206 -> 4.5282
KoCulture exact/prefix10             0/10, 0/10
EOS                                  0/10
```

전체 통합 데이터를 30 epoch 학습해도 valid loss는 `5.8475 -> 4.2104`로
내려갔지만 출력은 `ㅋㅋㅋㅋ`, `난 난 난` 등으로 붕괴했고 exact/prefix10은
`0/10`이었다. KoCulture만 분리한 10 epoch 학습도 `0/10`이었다.

따라서 현재 문제는 단순한 epoch 부족, EOS 가중치, source mixing 하나로
해결되지 않는다. train 질문과 test 질문이 대부분 새 문장이고 답변도 고유해
순수 생성 모델의 exact gate는 특히 강한 기준이다. 다음은 retrieval/copy
보조 경로를 별도 모델로 구현해, 질문 관련성은 회복하면서 순수 생성과의
차이를 수치로 비교한다.

## 224. retrieval/copy 보조 baseline

외부 의존성 없이 질문 character 3-gram Jaccard 검색기를 구현하고 TDD를
통과시켰다. train 질문에서 가장 비슷한 질문의 답변을 그대로 반환했다.

KoCulture test 10행 결과:

```text
exact/prefix10                         0/10, 0/10
답변 character overlap                 0.088
```
...
```text
exact/prefix10                         0/10, 0/10
답변 character overlap                 0.178
```

단순 character retrieval만으로는 의미 연결도 충분하지 않았다. 따라서 이를
최종 router에 바로 연결하지 않는다. 향후 retrieval을 쓰려면 한국어 의미
embedding 또는 source/category 필터를 추가하고, 검색 결과를 작은 생성 모델의
입력에 명시적으로 넣어 copy 여부를 별도 검증해야 한다.

## 225. IDF retrieval 및 retrieval-augmented decoder

character 3-gram Jaccard의 흔한 표현 편향을 줄이기 위해 IDF 가중치와
역색인을 추가했다. source/category가 같은 후보만 검색하도록 제한했다.

```text
KoCulture IDF retrieval answer overlap       0.102
기존 Jaccard answer overlap                   0.088
```

검색 결과를 `참고 질문 / 참고 답변 / 현재 질문` 형식으로 train-only
augmentation하고, 2M 이하 subword decoder를 10 epoch 학습했다.

```text
valid loss                                   5.4261 -> 4.3105
KoCulture exact/prefix10                     0/10, 0/10
EOS                                          10/10
주요 출력                                    '아서 듯', '헐 듯', ㅋㅋ 반복
```

IDF와 참고 문맥을 넣어도 작은 decoder가 참고 답변을 복사하거나 현재 질문에
맞게 변형하지 못했다. 따라서 retrieval-augmented 순수 생성도 현재 gate를
통과하지 못했다.

## 226. hidden128·2층 장기 학습 상한

2M 제한 안에서 hidden 128, embedding 32, 2 layer, 4 heads 모델을 20 epoch
학습했다. 파라미터 수는 631,936개다.

```text
valid loss                  5.3887 -> 3.9822
KoCulture exact/prefix10    0/10, 0/10
EOS                         9/10
출력                        정상 단어 일부 + `싶다`, `블랙스`, `걍`, `난` 반복
```

모델 용량과 학습량을 늘려도 질문별 조건부 의미 생성은 통과하지 못했다.
현재까지 순수 decoder, source-balanced 학습, EOS weighting, 단일 유형 학습,
retrieval-augmented 학습을 모두 분리 검증했으나 의미 gate는 실패 상태다.

## 227. sampling 및 train-generation 진단

hidden128·2층 모델을 temperature 0.8, top-k 20으로 샘플링해도 한국어 단어가
부분적으로 섞인 비문과 반복이 나왔다. KoCulture test exact/prefix10은 여전히
`0/10, 0/10`이었다.

같은 모델을 train 첫 10행에 넣어도 자유 생성 exact가 `0/10`이었다. 따라서
현재 실패는 test 일반화만의 문제가 아니라 teacher-forcing loss와 실제
autoregressive rollout 사이의 간극, 즉 작은 모델의 exposure bias/조건 유지
실패가 핵심이다.

## 228. scheduled sampling A/B

answer prefix 일부를 이전 예측으로 치환하는 scheduled sampling을 prompt에는
적용하지 않도록 구현하고 TDD를 통과시켰다.

```text
scheduled sampling probability       0.2
10 epoch valid loss                  6.0513 -> 4.5991
KoCulture 자유생성                   0/3 exact, 0/3 prefix10
출력                                  `ㄹㅇ` 뒤 공백 반복, ㅋㅋ 반복
```

teacher-forcing과 rollout 간극을 직접 줄이는 방식도 현재 모델에서는
개선되지 않았다.

## 229. 공식 hybrid gate 재검증 및 수도 변형 수정

history에 이미 기록된 copy head 실험은 순수 neural keyword 개선 `0.0`으로
실패했으므로 반복하지 않았다. 대신 현재 공식 평가에서 `대한민국의 수도는`
변형이 기존 reasoning router에 없어 `현재 확인할 수 없습니다.`로 빠지는
결함을 발견했다.

router에 한국·대한민국·우리나라 + 수도 조합을 추가하고 회귀 테스트를 먼저
작성했다.

```text
router 단위 테스트                         통과
공식 hybrid generation cases               20/20 keyword
keyword_hit_rate                           1.0
repetition_3gram_ratio                     0.0
전체 pytest                                59 passed, 3 skipped
hybrid regression                          PASS cases=22
```

현재 실제 사용 hybrid 경로는 공식 20개 generation gate와 22개 regression
gate를 통과한다. 단, 순수 dialogue decoder는 별도 gate에서 여전히 실패하므로
순수 모델이 이해를 획득했다고 판정하거나 새 pure checkpoint로 router를
교체하지 않는다.

## 231. pretrain → SFT 순서 검증

언어 prior 부재를 해결하기 위해 두 가지 pretraining 초기화를 검증했다.

```text
wiki 100k pretrain valid loss             4.8195
wiki pretrain -> SFT 10ep valid loss      5.3021 -> 4.5659
chat Q/A pretrain 5ep valid loss          5.6979 -> 4.6595
chat pretrain -> SFT 10ep valid loss      4.7680 -> 4.4602
```

하지만 두 모델 모두 KoCulture test 자유생성 exact/prefix10이 `0/10, 0/10`이고,
출력은 `ㅋㅋ`, `ㅇㅇ`, `헐`, 공통 token 반복으로 붕괴했다. pretraining은
teacher-forced loss와 초기 문장 prior를 개선했지만 질문 조건을 자유생성에
연결하지 못했다. 따라서 다음부터는 단순 pretrain 양을 늘리는 실험을 중단하고,
조건부 응답을 직접 선택하는 구조 또는 hybrid gate를 기준으로 판단한다.

## 232. compact seq2seq encoder-decoder A/B

질문 정보를 encoder가 보존하고 답변 decoder가 cross-attention으로 참조하는
seq2seq Transformer를 추가했다. embedding 16, hidden 64, 1 layer, 4 heads로
파라미터를 2M 이하로 유지했고 구조·decoder causality 테스트를 통과했다.

```text
5 epoch valid loss                   5.7394 -> 4.8006
20 epoch valid loss                  5.7790 -> 4.2326
KoCulture 20 epoch exact/prefix10    0/3, 0/3
EOS                                  3/3
출력                                  '헐 괜찮아', '난 난 난' 반복
```

encoder-decoder도 질문 조건을 안정적으로 보존하지 못했다. 따라서 현재까지
random decoder, pretrain→SFT decoder, retrieval-augmented decoder, scheduled
sampling, seq2seq decoder가 모두 순수 exact gate를 통과하지 못했다. 실제 사용
hybrid gate는 229~231의 통과 상태를 유지한다.

## 230. 실제 실행 래퍼 복구 및 smoke test

history에 기록돼 있던 `scripts/run_hybrid_chat.sh`가 현재 worktree에는 없어
실제 사용 명령으로 재현되지 않는 상태였다. 공식 checkpoint 3종을 기본값으로
묶은 실행 래퍼를 추가했다.

```text
dialogue:  hrm_context_copy_pure_dialogue_v2_12ep_best.pth
intent:    hrm_intent_pure_v3_best.pth
reasoning: hrm_context_reasoning_order_finetune_2ep_best.pth
입력       대한민국의 수도는
출력       서울입니다.
```

래퍼 smoke test는 통과했다. memory 파일은 현재 worktree에 없어 기본 로드하지
않으며, 공식 20-case·22-case gate 결과는 229번 수치를 유지한다.

## 233. seq2seq 용량 A/B

질문 encoder와 답변 decoder를 유지한 채 hidden 128, embedding 32, 2 layers로
확장했다. 파라미터 수는 1,029,504개로 2M 제한 안이다.

```text
10 epoch valid loss                  5.4027 -> 3.9977
KoCulture 자유생성                   0/3 exact, 0/3 prefix10
EOS                                  3/3
출력                                  `헐 진짜?`, `엄마`, `괜찮아` 반복
```

용량을 늘린 seq2seq도 pure 의미 gate를 통과하지 못했다. 현재 실패는 단순
hidden size·layer 수 부족으로 설명되지 않는다.

## 234. seq2seq beam search A/B

seq2seq hidden128·2층 checkpoint에 beam size 5 decoding을 적용했다.

```text
KoCulture 3행 exact/prefix10       0/3, 0/3
EOS                                3/3
출력                               일부 자연스러운 `헐 진짜? 괜찮아`와
                                   `뿡뿡`·`그래도` 반복 혼재
```

beam search도 학습된 분포의 의미 조건을 복구하지 못했다. 따라서 pure
decoder의 남은 문제는 greedy decoding 하나가 아니다.

## 235. 한국어 pretrained baseline 확인

새로 다운로드한 한국어 KoGPT2 base와 cache에 있던 Qwen2.5-0.5B-Instruct를
teacher 기준으로 짧은 자연 대화 4행에 비교했다.

```text
Qwen2.5-0.5B       일반적인 안내·부정확한 응답, 한국어 대화 조건 미흡
KoGPT2 base        한국어 token 복원은 정상이나 Q/A prompt에서
                   `챕터 3장 추가`, `#미미누` 반복
```

KoGPT2는 한국어 prior는 확인했지만 instruction 모델이 아니며, Qwen은
최종 작은 모델 목표에 비해 크고 현재 응답 품질도 기준에 미달했다. 따라서
둘을 최종 checkpoint로 연결하지 않는다.

## 236. 한국어 KoGPT2 125M SFT 기준선

한국어 pretrained prior의 효과를 확인하기 위해 KoGPT2 base를 다운로드된
SFT 5,000행에 1 epoch fine-tune했다. 이 모델은 2M 이하 최종 후보가 아니다.
...
한국어 pretrained prior만으로도 올바른 조건부 대화가 생기지 않았고, 125M
모델도 짧은 SFT에서는 반복 과적합했다. 따라서 작은 모델의 문제를 단순히
한국어 tokenizer 또는 parameter count로 설명할 수 없다.

## 237. 순수 생성 실패 해결 계획 재정립

현재 문제는 한글을 출력하지 못하는 문제가 아니라, 질문 조건을 유지하면서
답변을 선택하지 못하는 문제로 확정했다. random decoder, pretrain→SFT,
retrieval-augmented decoder, scheduled sampling, seq2seq, beam search,
KoGPT2 SFT를 이미 실행했으므로 같은 조건의 추가 학습은 중단한다.

앞으로는 다음 순서로 진행한다.

```text
1. 현재 pure/hybrid 기준선 보고서 고정
2. 다운로드된 실제 train row 1개 과적합 및 free-running 확인
3. 1행 실패 시 target shift·BOS/EOS·generate 경로 수정
4. 1행 성공 후 질문 encoder·답변 selector·소형 generator로 문제 분리
5. 실제 다운로드 데이터만 source/category 균형화
6. syllable generator와 subword generator를 2M 이하 A/B
7. 별도 holdout pure gate 평가
8. hybrid 20/20·22/22 회귀와 전체 테스트
```

pure gate는 free-running 정답률 80% 이상, 평균 3-gram 반복률 0.20 이하,
EOS 도달률 90% 이상, 깨진 출력 0이다. 이를 못 넘으면 hybrid를 실제 사용
경로로 고정하고 pure generator는 연구 후보로만 둔다. teacher-forced loss만
낮아진 모델은 성공으로 판정하지 않는다.

## 238. 기준선·정렬 감사·1행 과적합 재검증

현재 pure/hybrid 기준선과 다운로드 데이터 정렬을 다시 실행했다.

```text
pure HRM mean 3-gram repeat              0.000
pure HRM max 3-gram repeat               0.000
pure HRM bad-char ratio                  0.000
hybrid generation cases                  20/20
hybrid keyword_hit_rate                  1.000
hybrid repetition_3gram_ratio            0.000
hybrid regression                        PASS cases=22
```

pure 기준선 스크립트는 현재 exact·EOS를 산출하지 않으므로 위 pure 수치만으로
통과 판정하지 않는다. 실제 다운로드 natural valid 50행에서 기존 후보를
재평가한 결과는 다음과 같다.

```text
full syllable 3 epoch                    exact 0/50, prefix10 0/50
HRM context copy                         exact 0/50, prefix10 0/50
subword 30 epoch                         exact 0/50, prefix10 0/50, EOS 33/50
```

반면 실제 다운로드 train 1행 과적합 checkpoint는 구조별 차이를 보였다.

```text
hrm_single_overfit_v2                    exact 1/1
full_syllable_single_overfit             exact 1/1
hrm_single_overfit                       exact 0/1
```

따라서 target shift 정렬 자체는 정상이고, full-data 조건부 선택·free-running
안정성이 실패 지점이다. `audit_sft_alignment.py`가 dataset root를 직접 받으면
`train/` split을 찾지 못해 `KoJamoDataset.samples` 오류를 내던 문제도 발견했다.
회귀 테스트 2개를 먼저 추가한 뒤 root→train split 자동 해석을 수정했고,
root 경로 감사 결과는 다음과 같다.

```text
rows                                    20199
first active target index               43
first active target                    돈
last target                            newline EOS
prompt mask                            정상
answer mask                            정상
```

다음 실행은 selector/generator 분리 구조의 계약 테스트와 실제 다운로드
holdout 평가다. pure gate는 아직 미통과 상태다.

## 239. 조건부 selector 계약 및 평가기 안정성

질문 조건과 답변 선택을 분리하기 위해 다운로드 train 전용
`ConditionalSelector`를 추가했다. exact 질문 우선, fuzzy character TF-IDF
후보, pair 제외, 낮은 점수 거절을 계약 테스트 3개로 고정했다.

```text
selector 계약 테스트                     3 passed
downloaded test 100행 exact              0/100
downloaded test 1509행 exact              0/1509
downloaded test 1509행 prefix10          13/1509
```

단순 retrieval만으로는 질문 의미 일반화가 되지 않는다. 따라서 selector는
최종 pure 모델이 아니라 다음 구조 실험의 비교 기준선으로만 둔다.

full-syllable 평가기는 긴 질문에서 `sequence length 257 exceeds max_len 256`
으로 중단되는 결함이 있었다. 최근 context window를 사용하는 테스트를 먼저
실패시킨 뒤 수정했다.

```text
generation context test                  1 passed
short test 평가                          exact 0/66, prefix10 0/66
```

평가가 중단되지 않도록 수정됐지만 pure 성능은 개선되지 않았다.

## 240. selector 단계 후 회귀 검증

selector 계약과 full-syllable 평가기 수정 후 전체 검증을 다시 실행했다.

```text
pytest                                  68 passed, 3 skipped, 11 warnings
py_compile scripts/*.py                 통과
hybrid generation                       20/20, keyword_hit_rate 1.000
hybrid regression                       PASS cases=22
```

hybrid 공식 경로는 유지됐다. pure 모델은 실제 downloaded short/natural
holdout exact gate를 통과하지 못했으므로 최종 pure 성공으로 판정하지 않는다.
다음 단계는 selector를 답변 복사 경로로 채택하는 것이 아니라, 질문 encoder와
답변 후보 선택을 학습하는 2M 이하 A/B 실험이다.

## 241. 2M 이하 dual selector A/B

다운로드 chat train만으로 질문 encoder와 답변 encoder를 대조학습했다. 답변은
새로 만들지 않고 train answer bank에서 선택했다. 두 구조 모두 parameter 수는
2M 이하이며, valid/test 질문은 train answer bank으로만 평가했다.

```text
subword dual encoder, valid               exact 1/2427, prefix10 11/2427
syllable dual encoder, valid              exact 1/2427, prefix10 6/2427
subword dual encoder, test                exact 0/1509, prefix10 7/1509
syllable dual encoder, test               exact 0/1509, prefix10 3/1509
```

둘 다 pure gate와 거리가 크다. 단순 selector 또는 dual encoder만으로는
질문 의미를 답변 후보에 연결할 수 없으므로 최종 구조 후보에서 제외한다.
이 결과로 현재 실패는 decoder 하나의 문제가 아니라, 다운로드 데이터의
질문-답변 관계가 작은 모델이 일반화할 수 있을 만큼 반복·정렬되어 있지 않은
문제까지 포함한다.

## 242. 다운로드 category별 전문 모델 A/B

사용자 계획에 따라 합성 없이 `friendly_counseling` 원문 category만 분리했다.
...
기존 dialogue checkpoint를 초기값으로 사용해 같은 크기의 HRM 전문 모델을
1 epoch 학습했다.

```text
train loss                               1.8431
valid loss                               1.6370
전문 모델 순수 test 20행                  exact 0/20, prefix10 0/20
혼합 baseline 순수 test 20행              exact 0/20, prefix10 0/20
전문 모델 dialogue variants               12/12
```

전문 모델은 runtime 규칙 라우터가 제공하는 12개 응답에는 문제없이 연결되지만,
순수 자유생성은 개선하지 못했다. 따라서 category 분할만으로 pure gate를
해결하지 못하며, 전문 checkpoint를 현재 hybrid 기본 경로로 교체하지 않는다.

## 243. category 실험 후 최종 회귀

category split·dual selector 추가 후 기존 runtime을 다시 검증했다.

```text
pytest                                  71 passed, 3 skipped, 11 warnings
py_compile scripts/*.py                 통과
hybrid regression                       PASS cases=22
```

현재 hybrid 기준은 유지된다. pure gate는 다음 수치 때문에 미통과다.

```text
실제 downloaded holdout pure exact        0/50, 0/66
dual selector test exact                  0/1509
category specialist pure exact            0/20
```

따라서 현재까지의 증거상 더 많은 동일 데이터 학습은 해결책이 아니다. 다음
구조 변경은 생성 모델을 억지로 늘리는 대신, 답변 유형별 검증 가능한 출력
계약과 공통 언어 생성기를 결합하는 방향으로 제한한다.

## 244. GRU encoder-decoder + attention A/B

Transformer seq2seq와 다른 구조로 2M 이하 compact GRU encoder-decoder와
질문 attention을 구현했다. 실제 downloaded short train 8,771행으로 3 epoch
학습했다.

```text
parameter limit                           < 2M
epoch 1 train/valid loss                  6.3533 / 5.8803
epoch 2 train/valid loss                  5.7477 / 5.6443
epoch 3 train/valid loss                  5.4620 / 5.3751
short test 20행 exact/prefix10            0/20, 0/20
```

초기 generation은 매 token마다 encoder를 재계산하는 구현 결함이 있어
`decode_step`을 테스트-first로 추가했다. 수정 후 평가도 `0/20`으로 동일했다.
따라서 GRU·attention 역시 pure gate를 통과하지 못했다.

## 245. GRU 1행 과적합 원인 분리

GRU 구조가 질문-답변 조건을 전혀 학습하지 못한 것인지 확인하기 위해 실제
다운로드 train 1행으로 150 epoch 과적합을 실행했다.

```text
epoch 1 valid loss                      7.5312
epoch 150 valid loss                    0.0649
free-running exact                     1/1
출력                                  원문 답변과 동일
```

따라서 GRU target shift·attention·EOS 경로는 1행에서는 정상이다. full
downloaded 데이터에서의 `0/20`은 구현 불능이 아니라 질문 표현 일반화 실패다.
1행을 더 반복하는 것은 pure gate 해결책이 아니므로 추가 단일-row 학습은
중단한다.

## 255. 일상형 한국어 사칙연산 해석기 추가

현재 산수 specialist는 `[산수]` 태그와 일부 정형 표현만 처리했다. 일상
질문을 계산식으로 바꾸는 작은 한국어 해석기를 추가했다.

추가 파일:

```text
scripts/korean_arithmetic_nlu.py
tests/test_korean_arithmetic_nlu.py
```

지원 예:

```text
사과 3개에 2개를 더하면?                 -> 3+2=5개
사과 10개 중에서 3개를 먹으면?            -> 10-3=7개
연필 4자루씩 3묶음이면?                  -> 4×3=12자루
12000원을 4명에게 똑같이 나누면?         -> 12000÷4=3000원
7천 원에서 2천5백 원을 빼면?             -> 7000-2500=4500원
250만원에서 30만원을 쓰면?               -> 2200000원
```

`편의점에서`의 장소 표현을 뺄셈으로 잘못 읽는 문제가 있어 숫자 바로 앞의
`에서`만 뺄셈으로 인정하도록 수정했다. 연산이 명확하지 않은 문장은
`None`을 반환해 임의 계산하지 않는다.

검증:

```text
새 한국어 산수 테스트                         6 passed
전체 pytest                                  105 passed, 3 skipped
hybrid regression                            PASS cases=22
py_compile                                   통과
```

결론: 현재 구조는 일상형 간단 사칙연산을 처리할 수 있는 운영 경로가 됐다.
이는 순수 신경망이 계산을 이해했다는 뜻이 아니라, 작은 한글 모델이 질문의
표현을 해석하고 정확한 산술 specialist가 계산하는 hybrid 성공이다. 아직
한글 수사(`두 개`, `열다섯 개`), 소수·분수, 시간 단위가 섞인 문장,
세 단계 이상 복합식은 지원 범위 밖이다.

## 256. 학습 데이터 밖 일상형 산수 대화 평가

학습·평가 데이터에 없는 새 문장 10개를 현재 실제 `generate()` 경로에 넣어
검증했다. 결과는 다음과 같다.

```text
문장 단위 exact match                         5/10
계산 숫자까지 맞음                            6/10
```
...
```text
물병을 한 상자에 3개씩 넣었고 상자가 4개야.  -> 3×4=12개
책 9권 중 친구에게 4권을 빌려줬어.           -> 9-4=5권
12000원에 8000원을 더 모았어.                -> 20000원
주차비 3000원과 식사비 8500원을 합치면?      -> 11500원
2와 7을 합치면?                              -> 9
```
...
```text
5만원에서 1만2천원을 꺼냈어.                 -> 덧셈으로 오판
귤 6개를 두 사람에게 나누면?                 -> 자유생성 실패(한글 수사)
쿠키 24개를 6명에게 나누면?                  -> 4명으로 단위 오판, 정답은 4개
연필 5자루를 동생에게 줬어.                  -> 되묻기, 양도 표현 미지원
```

`2,500원+1,200원`은 실제 출력이 `3700원`으로 계산은 맞았으나, 평가 기대값의
쉼표 형식 차이로 exact에는 포함되지 않았다. 여러 턴에서 “공책 3권을 샀다”
뒤에 “한 권 가격은 1800원”처럼 이전 수량을 이어 계산하는 것은 아직 실패했다.

결론: 새로운 단일 문장의 간단한 계산은 일부 일반화되지만, 현재 모델은
일상 한국어 산수 모델로 완성되지 않았다. 다음 우선순위는 `두/세/네` 같은
한글 수사, `줬다/꺼냈다/남았다` 상태 변화, 나눗셈 결과 단위, 이전 턴의
수량·가격을 연결하는 계산 상태 메모리다.

## 250. 추천 순서에 따른 슬롯 계층 실행

distillation을 제외한 추천 순서 중 1순위 의미 계획 계층을 재검증했다.

```text
공식 의미 계획 평가                     20/20
계획 인식률                             1.000
키워드 적중률                           1.000
semantic-plan 단위 테스트               8 passed
hybrid 회귀                             22/22 PASS
```

2순위로 기존 한글 계획기에 보수적 의미 슬롯과 대화행위 분류를 추가했다.
추출 슬롯은 `location`, `item`, `activity`, `target`, `time`, `emotion`이며,
학습된 자유생성기가 아니라 검사 가능한 중간 계약이다.

추가 검증 결과:

```text
새 표현 슬롯 단위 테스트                 3 passed
기존 semantic-plan 테스트                8 passed
hybrid 회귀                             22/22 PASS
```

새로운 전체 대화 평가에서는 계획 계층만으로 충분하지 않았다.

```text
ko_unseen_dialogue                       plan 0.667 / keyword 0.400
ko_dialogue_v2_unseen                   plan 0.533 / keyword 0.133
ko_real_unseen                          plan 0.487 / keyword 0.000
```

따라서 슬롯 계층은 구조적 보조 기능으로는 통과했지만 순수 자유생성 문제를
해결했다고 판정하지 않는다. 계획에 따라 다음 denoising seq2seq 실험으로
넘어가기 전에, 위 unseen 실패를 기준선으로 보존한다. denoising 실험은 아직
실행하지 않았다.

## 251. denoising seq2seq 실행 결과

다음 추천 단계로 다운로드된 한국어 데이터에서 2M 이하 denoising seq2seq를
학습했다. distillation은 사용하지 않았다.

```text
구조                                     CompactSeq2SeqLM
parameter                                263,104
학습 데이터                              downloaded_sft 2,000행
denoising valid loss                     6.7629 -> 6.0130
denoising-only free generation           exact 0/20, prefix10 0/20, EOS 0/20
```

denoising checkpoint를 질문-답변 SFT 초기값으로 사용해 추가 학습했다.

```text
SFT valid loss                           5.9569 -> 5.8225
free generation                          exact 0/20
prefix10                                 0/20
EOS                                      0/20
```

출력은 빈 문자열 또는 `용`, `날` 등의 단어 반복이었다. denoising은 언어
표현 사전 초기화에는 도움을 주지만, 현재 작은 모델의 질문 조건 보존과
의미 기반 자유생성을 해결하지 못했다. 따라서 denoising 단독·단순 초기화
SFT를 최종 구조로 선택하지 않는다. 생성 목표를 더 작은 유형별 계약으로
분해하거나 hybrid 경로를 유지해야 한다.

## 252. 계획 기반 대화 hybrid 1단계

hybrid를 대화형으로 발전시키기 위해 `scripts/dialogue_hybrid.py`를 추가했다.
구조는 다음과 같다.

```text
질문 → 의미 계획 → 대화행위·슬롯 → 구조화 메모리 → 후보 답변 → specialist/응답
```

지원한 기능:

```text
인사·도움·위로·조언 후보 답변 2개
이름·위치·활동·물건 메모리 저장
새 표현의 메모리 조회
계산·순서·자모·사실·기억 specialist 우선 처리
```

`scripts/chat.py`에는 `--dialogue-manager` 옵션으로 연결했다. 기본 경로는
변경하지 않고, 옵션을 켠 경우에만 열린 대화의 계획 기반 fallback을 사용한다.

검증:

```text
대화 manager 단위 테스트                 6개 포함 관련 테스트 11 passed
hybrid 회귀                             22/22 PASS
전체 pytest                              95 passed, 3 skipped
py_compile                              통과
```

새 대화에서 인사·감정 공감·조언·사실 응답·활동 기억 조회가 동작했다. 다만
현재 후보 답변은 규칙 기반이며, 아직 학습된 의미 선택기가 아니다. 다음 단계는
실제 다운로드 대화 데이터에 대화행위와 슬롯 label을 부여하고, 후보 선택기를
학습·평가하는 것이다.

## 253. 대화행위 조건부 후보 selector 실험

다운로드된 `downloaded_chat_sft` 데이터에 질문 표현 기반 대화행위 약한 라벨을
붙이고, 같은 대화행위 후보군 안에서 문자 n-gram 답변을 검색했다.

```text
holdout                                2,427행
기존 selector exact                     1/2427
기존 selector prefix10                  20/2427
조건부 selector exact                   1/2427
조건부 selector prefix10                19/2427
```

약한 대화행위 조건은 검색 후보를 줄였지만 의미 일반화 성능을 높이지 못했다.
따라서 이 selector는 운영 hybrid 경로에 연결하지 않고 연구 기준선으로만
보존한다. 다음 후보 선택기는 질문-답변 의미 쌍을 직접 학습하는 dual encoder를
사용하되, 대화행위·슬롯을 추가 loss로 함께 학습해야 한다.

## 254. 대화행위 보조 dual encoder 실행 결과

질문·답변 임베딩 대조학습에 질문·답변 대화행위 분류 loss를 추가한
`ConditionedDualEncoder`를 구현하고 다운로드 대화 2,000행으로 학습했다.

```text
parameter                                2M 이하
train loss                               5.6899 -> 4.4759
valid 후보 선택 exact                    1/2427
valid 후보 선택 prefix10                 1/2427
```

학습 loss는 감소했지만 보지 못한 질문의 답변 후보 선택은 개선되지 않았다.
대화행위 약한 라벨만으로는 의미 표현 일반화가 되지 않으므로 checkpoint는
연구 기준선으로 보존하고 운영 hybrid에는 연결하지 않는다. 다음에는 사람이
검증한 짧은 대화 pair와 슬롯 일치 loss를 사용해야 한다.

## 250. 자모 의미 인코더·대조학습·조건부 생성 실행

계획한 의미 학습 단계를 실제 다운로드 데이터로 실행했다. 합성 문장은
학습에 넣지 않았다.

```text
자모 사전학습 rows                    60,000
자모 복원 loss                       1.1707
일반 대조학습 valid category top-1   1.0000
일반 대조학습 test exact@1           0.024876
조건부 생성 train loss               1.4446 -> 0.9980
조건부 생성 official keyword hit     0.000
```

category 분류는 되었지만 질문 의미에 맞는 답변 검색과 자유 생성은 실패했다.
조건부 생성 출력은 `아아아...` 반복이었다. 한글 출력률만 높아지고 의미가
생기지 않았으므로 이 checkpoint를 최종 모델로 채택하지 않는다.

## 251. hard negative 의미학습 재실행

질문과 같은 category의 다른 답변을 in-batch negative로 사용하는 hard-negative
대조학습을 추가해 일반 대조학습과 비교했다.

추가 변경:

```text
scripts/train_hangul_semantic.py  --hard-negative
tests/test_hangul_semantic_training.py
```
...
```text
일반 대조학습 test exact@1          0.024876
일반 대조학습 test category top-1   0.972637
hard-negative test exact@1          0.017413
hard-negative test category top-1   0.467661
hard-negative 생성 keyword hit      0.000
```

hard negative를 추가해도 의미 검색과 자유 생성은 개선되지 않았다. category
분류와 단어 의미 조합은 다른 능력이라는 결론을 유지한다.

## 250. 자모 의미 인코더·대조학습·조건부 생성 실행

순수 자유 생성에서 단어 의미 조합이 가능한지 확인하기 위해 실제 다운로드
데이터로 계획한 학습 단계를 실행했다. 합성 문장은 학습에 넣지 않았다.

추가 파일:

```text
scripts/hangul_semantic_encoder.py
scripts/pretrain_hangul_semantic.py
scripts/train_hangul_semantic.py
scripts/hangul_semantic_generator.py
scripts/train_hangul_semantic_generator.py
scripts/evaluate_hangul_semantic_generator.py
tests/test_hangul_semantic_encoder.py
tests/test_hangul_semantic_generator.py
```

### 자모 사전학습

다운로드된 한국어 위키 문장으로 자모 track masking 복원 학습을 실행했다.

```text
rows                                    60,000
steps                                   235
masked reconstruction loss              1.1707
encoder parameters                      90,512
```

### 질문·답변 의미 대조학습

다운로드 SFT train 31,526행으로 질문 벡터와 답변 벡터를 대조학습했다.
category head도 보조 손실로 학습했다.

```text
epoch 1~5 loss                          5.3084 -> 2.5347
valid category top-1                    1.0000
valid exact@1                           0.000319
valid exact@10                          0.000319
test category top-1                     0.972637
test exact@1                            0.024876
test exact@10                           0.024876
```

category는 분류했지만 질문의 의미에 맞는 답변을 검색하는 능력은 매우 낮다.
category 정확도를 단어 의미 이해의 증거로 사용하지 않는다. 기존 dual
selector와 마찬가지로 단순 대조학습만으로는 충분하지 않았다.

### 의미 벡터 조건부 생성

대조학습 encoder의 문장 벡터를 자모 decoder의 초기 상태로 전달하는 생성기를
추가해 다운로드 train 31,526행을 3 epoch 학습했다.

```text
train loss                             1.4446 -> 0.9980
generator parameters                   140,698
official free-generation cases         20
keyword hit rate                       0.000
mean Hangul ratio                      0.9951
대표 출력                              아아아 아아 아아 ...
```

한글 문자는 출력하지만 의미를 반영하지 않고 같은 음절을 반복했다. 따라서
자모 복원 + category 보조학습 + 문장 대조학습만으로 단어 의미를 자유 생성에
전달하는 데 실패했다.

### 최종 검증

```text
pytest                                 86 passed, 3 skipped, 11 warnings
py_compile scripts/*.py                통과
```
...
```text
순수 learned 의미-조건부 자유 생성      미통과
한글 출력                               통과
의미 category 분류                     통과
질문-답변 의미 검색                     미통과
기존 한글 hybrid runtime               통과(20/20, 22/22)
```

따라서 현재 운영 후보는 기존의 `자모 기반 작은 모델 + 의미 계획 + 검증된
specialist` hybrid로 유지한다. 새 의미 encoder·generator checkpoint는 연구
실패 기준선으로 보존하며, 성공한 최종 모델이라고 부르지 않는다.

## 247. 짧은 유형별 holdout 재검증

유형별 학습 가능성을 더 좁히기 위해 다운로드된 `friendly_counseling` 중
질문 80자 이하·답변 80자 이하인 원문만 사용했다.

```text
train                                    462
valid                                    53
test                                     69
HRM 10 epoch valid loss                  2.0516 -> 1.4688
전문 모델 test 20행                      exact 0/20, prefix10 0/20
혼합 baseline test 20행                  exact 0/20, prefix10 0/20
```

답변 길이와 유형을 제한하고 validation loss를 낮춰도 자유생성 정답은 생기지
않았다. 따라서 현재 2M 이하 랜덤 초기화/소형 Korean tokenizer 계열은
다운로드 자연어 holdout의 조건부 의미 일반화에 실패한다. 동일 계열 추가
학습은 중단하고 hybrid contract 경로를 최종 운영 후보로 유지한다.

## 246. learned-only와 contract runtime 분리 측정

공식 20개 case에서 reasoning router를 끈 learned-only 경로를 다시 측정했다.

```text
learned-only keyword_hit_rate            0.100
learned-only repetition_3gram_ratio      0.000
learned-only full_hangul_ratio           0.878
learned-only avg_new_tokens              9.0
contract/hybrid keyword_hit_rate         1.000
```

반복률과 한글 비율만으로는 의미 이해를 판정할 수 없다. 현재 작은 모델은
한글 문장은 만들지만 질문 조건을 잃는다. contract runtime은 의미 gate를
통과하지만 pure neural gate로 부르지 않는다.

## 248. GRU scheduled sampling 최종 A/B

teacher-forcing과 free-running 차이를 줄이기 위해 decoder 입력의 50%를
직전 예측 token으로 교체하는 scheduled sampling을 추가했다.

```text
epoch 1 valid loss                      5.9446
epoch 5 valid loss                      5.2266
short holdout 샘플 출력                 빈 문자열 또는 EOS 즉시 종료
```

기본 GRU의 epoch 5 valid loss `5.3751`보다 개선되지 않았고 생성 출력은 더
나빠졌다. scheduled sampling도 pure gate 해결책이 아니므로 해당 계열을
최종 후보에서 제외한다.

## 249. 한글 의미 계획 구조 최종 판정

기존 실험의 결론에 따라 순수 생성 모델을 더 크게 학습하지 않고, 한글 내부
구조를 사용하는 의미 계획 계층을 추가했다. `scripts/hangul_semantic_plan.py`는
입력 음절을 초성·중성·종성으로 분해하고, 의도·주제·슬롯·답변 형식·사용할
도구를 중간 계약으로 만든다. 열린 대화는 learned 경로에 남기고, 확인 가능한
계산·자모·순서·기억·안전 응답만 specialist 경로로 보낸다.

추가 파일:

```text
scripts/hangul_semantic_plan.py
scripts/evaluate_semantic_plan.py
tests/test_hangul_semantic_plan.py
```

`scripts/chat.py`의 운영 생성 경로에 계획 계층을 연결했다. 계획 계층을 별도
평가한 결과:

```text
official cases                         20
plan known rate                        1.000
...
```text
hybrid generation                      20/20
hybrid regression                      PASS cases=22
pytest                                 82 passed, 3 skipped, 11 warnings
py_compile                             통과
```

같은 checkpoint의 learned-only 경로는 기존과 같이 질문 조건을 안정적으로
반영하지 못한다. 따라서 이 결과를 순수 neural free-generation 성공으로
판정하지 않는다. 기존 순수 holdout exact 결과도 `0/20`, `0/50`, `0/66`으로
변하지 않았다.

모델 checkpoint parameter는 `708,589`개로 2M 이하이다. 다만 최종 통과한
것은 `자모 기반 작은 모델 + 의미 계획 + 검증 가능한 specialist` 혼합 구조다.
순수 생성 모델 기준은 미통과이며, 현재 목표에 맞는 운영 구조는 hybrid로
확정한다. 동일 GRU·Transformer·selector·scheduled-sampling 계열 추가 학습은
중단한다.

## 257. 다양한 산수 데이터셋 교차 평가

현재 `try_reasoning_answer()` 경로를 세 validation 데이터셋에 적용했다.

```text
데이터셋                         행      계산 경로 감지   숫자 정답
arithmetic_qa                    200/200       200/200      200/200
normalized_qa                    746           337           29
normalized_qa_random             749           342           30
```

`arithmetic_qa`는 정형 산수라 계산 숫자는 전부 맞았다. 그러나 출력에 계산식이
추가되어 exact 문장 일치는 0개였고, 나눗셈 결과 단위가 `원` 대신 `명`으로
나오는 형식 문제도 있었다.

자연어 문제에서는 `21인치 x 12인치`를 덧셈으로 계산하고, `2400미터·150미터·
6바퀴`의 세 단계 문제를 앞의 두 숫자만 사용했으며, 분수 문제를 단순 덧셈으로
처리하는 오류가 발생했다.

핵심 원인은 데이터 양 하나가 아니다.

1. 현재 해석기는 숫자 두 개와 연산 하나를 가정한다.
2. `총`, `모두`, `에서`를 문맥 없이 연산 신호로 사용한다.
3. 비율·분수·다단계·관계 추론을 위한 중간 상태와 식 구성기가 없다.

결론: 정형 산수 계산기는 충분히 정확하지만, 다양한 데이터에서 실패하는
중심 문제는 한국어 문장을 안전하게 계산식으로 변환하는 의미 해석 계층이다.
불확실할 때 계산하지 않는 검증 계층도 필요하다.

## 258. 다양한 문장 대응을 위한 안전한 산술 해석기 확장

257번 평가의 원인을 반영해 `scripts/korean_arithmetic_nlu.py`를 확장했다.
새 기능은 다음과 같다.

```text
한글 수사                 두 사람, 한 명의 문맥 처리
명시 곱셈                 x, ×, 곱하기, ~씩, 묶음
다단계 상태 변화           A개에 B개를 넣고 C개를 꺼냄 -> A+B-C
분수 두 항                A개의 1/6 + B개의 1/3
안전 중단                 세 번째 수량을 버리는 모호한 계산 거부
단위 검증                 조각과 칼로리처럼 다른 단위의 뺄셈 거부
```

요일이 포함됐다는 이유만으로 순서 specialist가 실행되던 router 조건도
`순서·배열·정렬`이 실제로 있을 때만 실행하도록 수정했다.

재평가:

```text
데이터셋              행     감지     숫자 정답     잘못 계산
arithmetic_qa         200    200      200          0
normalized_qa         746     37        1         36
normalized_qa_random  749     35        2         33
```

정형 산수의 계산 정확도는 `200/200`으로 유지됐다. 자연어 데이터는 지원하지
않는 비율·분수·관계·다단계 문제를 대부분 거부하도록 바뀌었지만, 아직 해결한
것은 아니다. 현재 단계의 개선은 오답을 내는 것보다 안전하게 미지원으로
넘기는 개선이다.

검증:

```text
한국어 산수 단위 테스트       8 passed
전체 pytest                  107 passed, 3 skipped
hybrid regression            PASS cases=22
py_compile                   통과
```

현재 산술 모델은 `한국어 문장 해석기 + 정확한 계산기` hybrid specialist다.
다음 모델 확장은 비율·분수·다단계 식을 각각 별도 parser와 검증 데이터로
추가하는 방식이어야 하며, 전체 자연어 데이터를 그대로 학습시키는 방식은
현재 문제를 해결하지 못한다.

## 259. 현재 운영 모델 전체 구조 요약

현재 실제 실행 구조는 하나의 순수 생성 모델이 아니라 다음 hybrid pipeline이다.
...
사용자 입력
  ...
  -> 한글 자모·숫자·기호 입력 인코딩
  -> 의미 계획기
       계산/자모/순서/사실 -> 검증된 specialist
       인사/위로/조언      -> DialogueManager
       미확인 대화         -> 학습 모델 또는 안전 거부
  -> 한국어 답변
```

기본 대화 checkpoint는
`checkpoints/hrm_context_copy_pure_dialogue_v2_12ep_best.pth`다.
...
hidden         128차원
문맥 길이       256
입력 track      초성·중성·종성·기호·영어·숫자 6개
내부 구조       causal Transformer 문맥 인코더 + H/L GRU
출력 head       타입·초성·중성·종성·기호·영어·숫자
```

한글은 완성 음절을 바로 처리하지 않고 초성·중성·종성으로 분리한다.
예를 들어 `한`은 `ㅎ + ㅏ + ㄴ`으로 표현된다.

운영 순서는 `scripts/chat.py`의 `generate()`에 정의되어 있다. 계산·자모·
순서처럼 정답 검증이 가능한 요청은 자유 생성보다 먼저 specialist를 실행한다.
열린 대화만 작은 신경망 생성 경로로 넘어가며, 낮은 확신의 답변은 안전한
거부 또는 되묻기로 끝낸다.

현재의 강점은 작은 크기, 한글 자모 구조, 검증 가능한 계산·기억·순서 처리다.
현재의 약점은 순수 신경망 자유 대화, 복잡한 의미 조합, 긴 대화 상태 유지다.
따라서 이 checkpoint를 순수 한국어 지능 모델의 성공으로 판정하지 않고,
`작은 한글 생성 모델 + 의미 계획 + specialist + 제한적 대화 메모리` 구조로
기록한다.
