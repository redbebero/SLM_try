...
def build_sft_model(vocab_sizes, checkpoint_path=None, device=None):
    ...
    if state_dict is None:
        ...
        # Legacy Transformer checkpoints used 29 jong classes; current
        # tokenizer uses 28. Build model from checkpoint tensor shapes so old
        # checkpoints remain loadable without changing current tokenizer.
        checkpoint_vocab_sizes = list(vocab_sizes)
        for index, name in enumerate(("cho", "jung", "jong", "sym", "eng", "num")):
            weight = state_dict.get(f"emb_{name}.weight")
            if weight is not None and weight.ndim == 2:
                checkpoint_vocab_sizes[index] = weight.shape[0]
        if spec["variant"] == "transformer":
            model = KoJamoTransformer(
                vocab_sizes=tuple(checkpoint_vocab_sizes), emb_dim=spec["emb_dim"],
