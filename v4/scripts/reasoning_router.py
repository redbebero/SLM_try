...
def _arithmetic(prompt):
    ...
    if any(word in compact for word in ("먹었", "사용했", "썼", "주면", "빼면", "남")):
        answer = a - b
        return f"{a}-{b}={answer}이므로 정답은 {answer}개입니다."
    if any(word in compact for word in ("더했", "더하", "합치", "모으면")):
        answer = a + b
        return f"{a}+{b}={answer}이므로 정답은 {answer}개입니다."
    return None
...
def _jamo(prompt):
    ...
    final = ""
    final_match = re.search(r"(?:종성|받침)\s*(?:은|는|이|가)?\s*([ㄱ-ㅎ]+)", prompt)
    if final_match:
...
def _conversation_state(prompt):
    ...
    activity_matches = re.findall(
        r"(?:나는|저는)\s+(.{1,30}?)(?:해|한다|하고\s*있어|하고\s*있어요|하는\s*편|타)",
        earlier,
    )
...
def _safe_dialogue(prompt):
    ...
    if any(word in compact for word in ("우울해", "우울한", "가라앉아", "가라앉은", "힘들어", "힘든", "속상해", "속상한", "마음이무거워", "지쳤어", "지쳤다", "지친")):
    ...
    if "응원" in compact and any(word in compact for word in ("문장", "메시지", "말", "만들")):
        return "네가 해낼 수 있어. 오늘도 한 걸음씩 힘내자!"
...
def try_reasoning_answer(prompt):
    ...
    natural_arithmetic = solve_natural_arithmetic(latest)
    if natural_arithmetic is not None:
        return natural_arithmetic
    ...
    if "초성" in latest and "중성" in latest:
        ...
        if answer is not None:
            return answer
    if ("순서" in latest or "배열" in latest or "정렬" in latest) and any(
            word in latest for word in ("계절", "요일", "달", "숫자")):
    ...
    if "지구가 태양" in latest and "걸리는 시간" in latest:
        return "1년입니다."
    if ("수도" in latest
            and any(country in latest for country in ("대한민국", "한국", "우리나라"))):
        return "서울입니다."
