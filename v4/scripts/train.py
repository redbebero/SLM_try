...
def effective_warmup_steps(total_steps, configured_steps):
    """Keep finite smoke runs from spending most updates below useful LR."""
    return min(int(configured_steps), max(1, int(int(total_steps) * 0.05)))
...
def save_best_checkpoint(model, optimizer, scheduler, epoch, metric):
    ...
    print(f"🏆 Best checkpoint 저장: {save_path} (epoch={epoch}, generation={metric:.4f})")
...
@torch.no_grad()
def evaluate_autoregressive(model, dataloader, device, max_batches,
                            prompt_length=8, rollout=8):
    """Measure generation after feeding model predictions back as inputs."""
    model.eval()
    total_tokens = 0
    correct_tokens = 0
    correct_types = 0
    ...
    for batch_index, (x, y) in enumerate(dataloader):
        if batch_index >= max_batches:
            break
        x, y = x.to(device), y.to(device)
        prompt = x[:, :prompt_length].clone()
        target_start = prompt_length - 1
        previous = None
        for step in range(min(rollout, y.size(1) - target_start)):
            outputs = model(prompt)
            logits_type, logits_cho, logits_jung, logits_jong, logits_sym, logits_eng, logits_num = outputs
            pred_type = logits_type[:, -1].argmax(dim=-1)
            next_token = torch.zeros(x.size(0), 1, 6, dtype=torch.long, device=device)
            korean = pred_type == 0
            symbol = pred_type == 1
            english = pred_type == 2
            number = pred_type == 3
            next_token[korean, 0, 0] = logits_cho[:, -1].argmax(dim=-1)[korean]
            next_token[korean, 0, 1] = logits_jung[:, -1].argmax(dim=-1)[korean]
            next_token[korean, 0, 2] = logits_jong[:, -1].argmax(dim=-1)[korean]
            next_token[symbol, 0, 3] = logits_sym[:, -1].argmax(dim=-1)[symbol]
            next_token[english, 0, 4] = logits_eng[:, -1].argmax(dim=-1)[english]
            next_token[number, 0, 5] = logits_num[:, -1].argmax(dim=-1)[number]

            target = y[:, target_start + step]
            correct_tokens += (next_token[:, 0] == target).all(dim=-1).sum().item()
            total_tokens += x.size(0)
            correct_types += (pred_type == model._get_types(target.unsqueeze(1)).squeeze(1)).sum().item()
            if previous is not None:
                repeat_count += (next_token[:, 0] == previous).all(dim=-1).sum().item()
                repeat_total += x.size(0)
            previous = next_token[:, 0]
            prompt = torch.cat([prompt, next_token], dim=1)
    model.train()
    return {
        "token_acc": correct_tokens / max(1, total_tokens),
        "type_acc": correct_types / max(1, total_tokens),
        "repeat_rate": repeat_count / max(1, repeat_total),
    }
...
def train():
    ...
    val_data_dir = args.val_data_dir or VAL_DATA_DIR
    ...
    actual_epoch_batches = training_batch_limit(len(dataloader), args.limit_batches)
    finite_total_steps = (
        actual_epoch_batches * max(1, int(EPOCHS))
        if EPOCHS is not None else None
    )
    effective_warmup = (
        WARMUP_STEPS if finite_total_steps is None
        else effective_warmup_steps(finite_total_steps, WARMUP_STEPS)
    )
        ...
        if nums:
            ...
            if resume_mode and isinstance(checkpoint, dict) and "model" in checkpoint:
                ...
                if "T_max" in saved_scheduler_state:
                    expected_epoch_end = saved_epoch * len(dataloader) - effective_warmup
                    start_epoch = (
                        saved_epoch
    ...
    # 0->BASE_LR로 선형 램프업해서 초기 학습을 안정화한다. 유한 smoke run에서는
    # 전체 step의 5%로 줄여 warmup이 전체 실행을 먹지 않게 한다.
    global_step = 0
    ...
    try:
         while EPOCHS is None or epoch <= EPOCHS:
            ...
            for x, y in pbar:
                ...
                if not skip_warmup and global_step < effective_warmup:
                    warmup_lr = BASE_LR * (global_step + 1) / effective_warmup
            ...
            rollout_metrics = evaluate_autoregressive(
                model, val_dataloader, device, max_batches=min(8, VAL_BATCHES),
                prompt_length=min(8, SEQ_LENGTH // 2), rollout=min(8, SEQ_LENGTH // 2),
            )
            print(
                f"🧪 Autoregressive | token={rollout_metrics['token_acc']:.3f} "
                f"type={rollout_metrics['type_acc']:.3f} "
                f"repeat={rollout_metrics['repeat_rate']:.3f}"
            )
            generation_score = (
                1.0 - rollout_metrics["token_acc"] + rollout_metrics["repeat_rate"]
            )
            scheduler.step(generation_score)

            if generation_score < best_generation_score:
                best_generation_score = generation_score
                save_best_checkpoint(model, optimizer, scheduler, epoch, generation_score)
