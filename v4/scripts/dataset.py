    ...
    def __init__(self, data_dir="train_data", seq_length=50, stride=10, is_sft=False):
        ...
        if is_sft:
            ...
            if os.path.exists(mmap_path) and os.path.exists(mmap_meta_path):
                ...
                cache_matches_window = (
                    meta.get("seq_length") == self.seq_length
                    and meta.get("stride") == self.stride
                )
                if (
                    meta.get("pretrain_eos") is True
                    and "sample_starts" in meta
                    ...
                    return
                print("♻️ 기존 캐시는 EOS 또는 window 설정이 달라 재생성합니다.")
                    ...
                    for source in source_files:
                        with open(source, "r", encoding="utf-8") as f:
                            for line in f:
                                sample = line.strip()
                                if sample:
                                    # Newline is an explicit end-of-sample token.
                                    # The file newline remains a separator only.
                                    yield sample + "\n"
